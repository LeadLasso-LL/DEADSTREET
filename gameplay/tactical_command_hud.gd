...
func setup(p_view: Node) -> void:
	...
	faction_label = label(surface, Vector2(24, 10), Vector2(610, 18), "OPERATIVES", 12)
	...
	selection_label = label(surface, Vector2(680, 43), Vector2(444, 18), "SELECT UNITS", 10)
	...
	for row in [["hold", "HOLD", 82], ["push", "PUSH", 82], ["fall_back", "FALL BACK", 102], ["clear", "CLEAR ORDERS", 126]]:
		...
		buttons[id] = b
		x += row[2] + 6
	command_label = label(command_row, Vector2(x + 7, 7), Vector2(170, 18), "", 10)
	...
	for i in range(4):
		var labels = ["Ⅱ", "▸ ½×", "▶ 1×", "▶▶ 1½×"]
		var b = button(playback, labels[i], Vector2(114 + i * 54, 0), Vector2(50, 29))
		b.tooltip_text = ["Pause / resume · Space", "Slow motion · 0.5×", "Normal speed · 1×", "Fast forward · 1.5×"][i]
		b.pressed.connect(set_playback.bind(i))
		playback_buttons.append(b)
	var sound = CheckButton.new()
	playback.add_child(sound)
	sound.position = Vector2(332, 0)
...
func _process(delta: float) -> void:
	...
	for i in range(ordered.size()):
		...
		if p.is_alive:
			living += 1
	faction_label.text = "YOUR FORCE  /  %d ACTIVE · %d ELIMINATED" % [living, roster.size() - living]
	if not roster.is_empty():
		var p = b.get_participant(roster[0])
		if p.identity != null:
			faction_label.text = str(Factions.profile(p.identity.gang_archetype_id).get("name", "YOUR FORCE")).to_upper() + "  /  %d ACTIVE · %d ELIMINATED" % [living, roster.size() - living]
	...
	selection_label.text = "%d SELECTED  ·  SHIFT-CLICK / DRAG TO GROUP" % selected
	for id in buttons:
	...
	playback_buttons[0].text = "▶" if b.tactical_paused else "Ⅱ"
	command_label.text = "PAUSED" if b.tactical_paused else "%s× SPEED" % b.tactical_speed
	feedback_clock -= delta
	if feedback_clock <= 0.0:
		feedback_clock = .15
		feedback_label.text = "Select units to give orders" if c == null else c.feedback
		if c != null and selected == 1 and c.pending_command_id.is_empty():
			feedback_label.text += "  ·  " + unit_feedback(b, b.get_participant(c.selected_participant_id))
	queue_redraw()
...
func unit_feedback(b, p) -> String:
	...
	if not p.player_priority_target_id.is_empty():
		...
		if target != null and target.is_alive:
			...
			return "PRIORITY TARGET · engaging when ready"
