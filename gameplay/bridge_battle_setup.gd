...
static func apply(b,config: Dictionary) -> Dictionary:
 ...
 for side_id in [b.attacker_side_id,b.defender_side_id]:
  ...
  if side_id==b.attacker_side_id:
   # West-approach arrivals travel east on the lower carriageway. Never
   # use the oncoming lanes as overflow parking.
   for x in [34.,22.,10.]:
    for y in [35.,41.]:candidates.append(Vector2(x,y))
  else:
