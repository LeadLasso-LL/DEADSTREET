...
graph.append(''.join(f'[c{i}]' for i in range(len(shots)))+'concat=n=12:v=1:a=0,settb=AVTB,setpts=N/(30*TB),fps=30,hue=s=0,eq=contrast=1.12:brightness=-0.008:gamma=0.99,colorchannelmixer=rr=0.64:gg=0.64:bb=0.64,vignette=angle=PI/5[v]')
...
'[4:a]volume=-4dB[a]']
run(['-filter_complex_threads','2','-i',str(d/'montage_candidate.ogv'),'-loop','1','-framerate','30','-i',str(d/'preview_title.png'),'-loop','1','-framerate','30','-i',str(assets/'camera_ui.png'),'-loop','1','-framerate','30','-i',str(assets/'open_button.png'),'-ss','21','-i',str(assets/'B-22_Dead_Street.mp3'),'-filter_complex',';'.join(graph),'-map','[v]','-map','[a]','-frames:v','720','-t','24','-c:v','libx264','-preset','fast','-crf','19','-pix_fmt','yuv420p','-c:a','aac','-b:a','160k','-movflags','+faststart','-y',str(d/'DEAD_STREET_Action_Montage.mp4')],'review_preview')
run(['-i',str(d/'DEAD_STREET_Action_Montage.mp4'),'-f','null','NUL'],'decode_preview')
sheet=Image.new('RGB',(1920,1080));offset=0
for i,s in enumerate(shots):
 t=offset+min(.8,s[2]/60.);out=d/f'preview_{i:02d}.jpg'
 run(['-ss',str(t),'-i',str(d/'DEAD_STREET_Action_Montage.mp4'),'-frames:v','1','-vf','scale=480:270','-y',str(out)],f'preview_still_{i}')
...
data=(d/'DEAD_STREET_Action_Montage.mp4').read_bytes();parts=d/'transfer';parts.mkdir(exist_ok=True);manifest=[]
for i,start in enumerate(range(0,len(data),750000)):
 chunk=data[start:start+750000];name=f'part_{i:03d}.txt';(parts/name).write_text(base64.b64encode(chunk).decode());manifest.append({'name':name,'bytes':len(chunk),'sha256':hashlib.sha256(chunk).hexdigest()})
report={'frames':frames,'seconds':24,'size':[1280,720],'maps':sorted(sources),'shots':shots,'candidate_sha256':hashlib.sha256((d/'montage_candidate.ogv').read_bytes()).hexdigest(),'source_hashes':{n:hashlib.sha256((r/p).read_bytes()).hexdigest() for n,p in sources.items()},'preview':str(d/'DEAD_STREET_Action_Montage.mp4'),'bytes':len(data),'sha256':hashlib.sha256(data).hexdigest(),'parts':manifest,'review_type':'Edited montage preview using exact candidate and existing native title/UI coordinates; not a full intro capture'}
