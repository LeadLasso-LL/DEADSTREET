...
# Verify every decoded frame across the hold and blackout, including the top edge.
warning_data=subprocess.check_output(['ffmpeg','-v','error','-xerror','-i',str(v),'-vf','select=between(n\\,225\\,389)','-fps_mode','passthrough','-pix_fmt','gray','-f','rawvideo','-'])
warning=np.frombuffer(warning_data,np.uint8).reshape(165,720,1280)
steady=warning[15:53].astype('int16')
hold_error=np.abs(steady-steady[0]).mean((1,2))
assert float(hold_error.max())<1.0,('caution moved during full hold',float(hold_error.max()))
assert float(steady[0].mean())>40,('caution not fully visible',steady[0].mean())
assert int(warning[120:].max())<=2,('black hold is not full-frame black',int(warning[120:].max()))
assert int(warning[111:120,:8,:].max())<=12,('white sliver remains above blackout',int(warning[111:120,:8,:].max()))
assert int(warning[117:120].max())<=3,('zoom did not finish in clean black',int(warning[117:120].max()))
caution_checks={'full_brightness_hold':[8.0,9.25],'checked_still_frames':38,'maximum_hold_mean_pixel_error':float(hold_error.max()),'black_hold_frames_checked':45,'black_hold_max_pixel':int(warning[120:].max()),'top_edge_max_pixel_11_2_to_11_5':int(warning[111:120,:8,:].max()),'last_zoom_frames_max_pixel':int(warning[117:120].max())}
...
report={'status':'PASS','frames':1710,'duration':57,'width':1280,'height':720,'fps':30,'full_video_audio_decode':'PASS',
 'caution_checks':caution_checks,'black_hold_checked_frames':[345,360,389],'title_complete_frame':630,'title_correlation':title_corr,
