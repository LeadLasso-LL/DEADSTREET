...
from compose_opening import ROOT, run
...
def render():
    ...
    for n in range(360):
        ...
        if t>=6:
            ...
            elapsed=36+int(t);text(px+18,py+142,f'0:{elapsed:02d}',11,(157,169,173,255));text(px+pw-47,py+142,'2:35',11,(157,169,173,255))
    ...
    run(['ffmpeg','-v','warning','-loop','1','-framerate','30','-i',str(ROOT/'sandbox_backdrop.png'),
         ...
         '-map','[v]','-frames:v','360','-an','-c:v','libx264','-crf','18','-preset','fast','-threads','2','-y',str(ROOT/'sandbox_handoff.mkv')],'sandbox_handoff.log')
    print('SANDBOX_HANDOFF_RENDERED',flush=True)
