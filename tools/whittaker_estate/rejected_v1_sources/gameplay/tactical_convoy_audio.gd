 ...
 for side in [b.attacker_side_id,b.defender_side_id]:
  ...
  if identity=="whittaker" and b.battlefield_geometry.authored_layout_id=="whittaker_estate_v1":
   ...
   if clip!=null:
    ...
    player.set_meta("fixed_position",Vector2(151,68));player.set_meta("range",1450.)
  ...
  for v in candidates:
   ...
   player.set_meta("base_gain",-18. if identity=="stateline" else (-27. if identity=="trc" else -29.));player.set_meta("offset",count*2.35)
