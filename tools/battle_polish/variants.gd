...
var runtime
var b
var tests=[{"seed":2002,"order":"push","arrival":"close"},{"seed":2003,"order":"push","arrival":"close"},{"seed":2005,"order":"push","arrival":"medium"},{"seed":2011,"order":"focus_left","arrival":"medium"},{"seed":2017,"order":"push","arrival":"far"},{"seed":2023,"order":"push","arrival":"far"}]
var index=0
var reports=[]
var active=false
var seen={}
...
func start():
 ...
 var result=Scenario.setup(runtime,tests[index].seed,false,tests[index].arrival)
...
func _process(_dt):
 ...
 return false
func finish():
 var r={"seed":tests[index].seed,"order":tests[index].order,"arrival":tests[index].arrival,"time":b.elapsed_time_seconds,"phase":b.battle_phase,"winner":b.get_winning_side_id(),"shots":seen.size(),"max_quiet":maxf(max_quiet,b.elapsed_time_seconds-last_shot),"decisions":b.strength_events,"samples":samples}
 ...
 if index<tests.size():start()
  ...
  FileAccess.open("res://tools/battle_polish/results/variants.json",FileAccess.WRITE).store_string(JSON.stringify(reports,"  "));quit()
