extends SceneTree
const OUT="C:/Users/brand/OneDrive/Documents/dead-street/tools/caution_overlap_20260915/"
var scene
var errors=[]
var checks=0
var started_frame=0
...
func run():
	...
	var path_ok=true
	for i in range(595):
		var v=scene.caution.zoom_view_at(6.85+float(i)/120.0)
	...
	for i in range(5):
		...
		var v=scene.caution.zoom_view_at(scene.elapsed)
		var point=Vector2(640,360)+(scene.caution.CENTERS[i]-Vector2(v.x,v.y))*v.z
		check(Rect2(50,50,1180,620).has_point(point),"impact remains visible during zoom %d"%(i+1))
