...
manifest=json.loads((R.parent.parent/'assets/art/units/pixel_v1/manifest.json').read_text())
for clip,start in [('jog',manifest['clips']['walk']['start']),('death',manifest['clips']['death']['start'])]:
 seq=[]
 for i in range(24):
  ...
  seq.append(board)
 duration=30 if clip=='jog' else 62
