...
def mark(parent,w,reload=0):
 old(parent,w,reload);directions.E.SubElement(parent,directions.N+'circle',{'id':'abdomen_anchor','cx':str(equipment.DEFINITIONS[w]['muzzle'][0]),'cy':str(equipment.DEFINITIONS[w]['muzzle'][1])})
...
def find(node,parent):
 ...
 if node.get('id')=='abdomen_anchor':return list((m@np.array([float(node.get('cx')),float(node.get('cy')),1]))[:2])
 for child in node:
  result=find(child,m)
  if result is not None:return result
p=R.parent.parent/'assets/art/units/pixel_v1/abdomen.json';data={}
for k in range(3):
