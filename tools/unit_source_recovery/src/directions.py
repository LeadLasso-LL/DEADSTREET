...
import numpy as np
import build,equipment,outfits
...
def recolor(g,k,w=None):
 costume=outfits.spec(k,w)
 ...
 if costume:colors=outfits.lower_colors(costume)
 for el in g.iter():
  for key in ['fill','stroke']:
   if el.get(key) in colors:el.set(key,colors[el.get(key)])
def upper(k,q,d,w,settle=0,aim=0,kick=0,flash=False,crouch=0,fall=0,lean=0,reload=0,wounded=0,backward=0):
 costume=outfits.spec(k,w)
 ...
 out=E.Element(N+'g');angle=((11+s*.7) if side else (4+s*.7 if diag else s*.7))+(25 if side else 8 if diag else 4)*crouch+lean;angle=angle*(1-fall)+(({'E':-90,'W':-90,'S':0,'N':180,'NE':-150,'NW':150}.get(d,-32)) if backward else 90)*fall
 g=group(out,transform=f'rotate({angle}) scale(1 {(1-.12*crouch*(1-fall))*(1-(.56 if d in ["N","S"] else .34 if diag else 0)*fall if backward else 1)})')
 ...
 if backward:
  lift=build.B.smooth(min(1,backward/.22));relax=build.B.smooth(max(0,min(1,(backward-.24)/.52)))
  right=right*(1-lift)+(np.array([-21.,-47.])*(1-relax)+np.array([-27.,-16.])*relax)*lift
  left=left*(1-lift)+(np.array([22.,-47.])*(1-relax)+np.array([28.,-15.])*relax)*lift
  wr_near,wr_far=right,left
  en=en*(1-lift)+(np.array([-22.,-34.])*(1-relax)+np.array([-22.,-23.])*relax)*lift
  ef=ef*(1-lift)+(np.array([22.,-34.])*(1-relax)+np.array([23.,-22.])*relax)*lift
 ...
 def gun(parent):
  ...
  if fall<.45 and backward<.15:equipment.draw(gg,w,reload)
  if flash:
   x,y=weapon["muzzle"]
  ...
  hand_skin,hand_hi=('#10181c','#30383b') if costume and costume.get('gloves') else (skin,hi)
  for hand in [right,left]:
   p(parent,f'M{xy(hand+[-1.7,-1.1])} Q{xy(hand+[0,-2])} {xy(hand+[1.8,-.3])} L{xy(hand+[1.5,2])} Q{xy(hand+[-.5,2.4])} {xy(hand+[-1.8,1])}Z',hand_skin,'#090f12',1.2)
   p(parent,f'M{xy(hand+[-.7,-.4])} L{xy(hand+[.8,.3])}','none',hand_hi,.65)
 ...
 cx=5 if side else (3 if diag else 0)
 if costume:
  outfits.torso(torso,costume,skin,hi,'S' if backward and fall>.6 and back else d)
  head=group(g,transform=f'translate({cx} {1-.2*s}) translate(0 -34) scale(.88) translate(0 34)')
  outfits.head(head,costume,skin,hi,'S' if backward and fall>.6 and back else d)
 else:
 ...
 if not back:
  if (side or d=='SW') and not wounded:
   ...
   outline_start=len(g)
   if costume:outfits.forearm(g,ef,wr_far,costume,skin,hi)
   else:
 ...
 if wounded and back:
  ...
  p(g,f'M{xy(left+[-2,-1])} Q{xy(left+[0,-3])} {xy(left+[2,0])} L{xy(left+[1,2])} {xy(left+[-2,1])}Z','#10181c' if costume and costume.get('gloves') else skin)
...
def make(k,q,d,w,settle=0,aim=0,kick=0,flash=False,crouch=0,fall=0,lean=0,reload=0,wounded=0,backward=0):
 # User-approved handedness swap: these views are true reflections.
 if d in ['SW','W']:
  root=make(k,q,'SE' if d=='SW' else 'E',w,settle,aim,kick,flash,crouch,fall,lean,reload,wounded,backward)
  ...
  return root
 if d in ['SE','SW']:
  root=build.make(k,q,w,settle,aim,kick,flash,crouch,fall,lean,reload,wounded,backward)
  if d=='SW':
   ...
   wrap.append(upper(k,q,'SW',w,settle,aim,kick,flash,crouch,fall,lean,reload,wounded,backward))
   low.set('transform','translate(128 0) scale(-1 1)')
  return root
 if d in ['E','W']:
  low,_,hip=lower_body.make_lower(q,settle=settle,crouch=crouch,fall=fall,wounded=wounded,backward=backward)
  if d=='W':low.set('transform','translate(128 0) scale(-1 1)')
 else:
  yaw=math.radians({'S':90,'N':270,'NE':293.578,'NW':246.422}[d]);low,hip=views.lower(q,yaw,settle=settle,crouch=crouch,fall=fall,wounded=wounded,backward=backward)
 recolor(low,k,w)
 ...
 wrap.append(upper(k,q,d,w,settle,aim,kick,flash,crouch,fall,lean,reload,wounded,backward));return root
