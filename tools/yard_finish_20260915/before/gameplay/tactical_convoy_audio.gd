...
func sync(b,poses: Dictionary,enabled: bool,duck: float,ending: float,battle_mix: float=0.0,victory_mix: float=0.0):
 ...
 for id in sources:
  ...
  # Owner-requested 10% linear arrival boost, scoped to this yard's incoming radio.
  if is_radio and v!=null and v.side_id==b.attacker_side_id and b.battlefield_geometry.authored_layout_id=="doble_ocho_yard_v1" and b.battle_phase not in ["active","resolved"]:
   player.volume_db+=linear_to_db(1.10)
