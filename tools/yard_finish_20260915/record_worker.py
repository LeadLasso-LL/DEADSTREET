...
for filename in ['audio_review.json']:
 report=json.loads((o/filename).read_text(encoding='utf-8'));assert not report['errors'],report
raw=o/'yard_revision_raw.avi';assert not raw.exists(),'Existing capture preserved'
paths=[p for folder in ['battle','campaign','core','gameplay'] for p in (r/folder).rglob('*.gd')]+[o/'showcase.gd',o/'capture_config.gd',r/'assets/art/doble_ocho/ground.png']
...
args=[r'C:\Users\brand\OneDrive\Documents\Godot\Godot_v4.7.2-stable_win64.exe','--path',str(r),'--resolution','1280x720','--position','0,0','--fixed-fps','30','--disable-vsync','--write-movie',str(raw),'--log-file',str(o/'capture_godot.log'),'--script','res://tools/yard_finish_20260915/showcase.gd','--','--record']
...
assert record['winner']=='attacker','Retain this non-winning take for evidence; owner requests a Ravicci winning take'
