extends SceneTree
func _initialize():
 var base="res://tools/arsenal_production/"
 var data: Dictionary=JSON.parse_string(FileAccess.get_file_as_string(base+"jobs.json"))
 var dest="res://assets/art/weapons/arsenal/"
 var atlases: Dictionary={}
 var clipped: Array=[]
 DirAccess.make_dir_recursive_absolute(base+"samples")
 for job: Array in data.jobs:
  var im=Image.new()
  if im.load_svg_from_string(FileAccess.get_file_as_string(base+"render_svg/"+str(job[0])+".svg"),2.)!=OK:push_error(str(job[0]));quit(1);return
 ...
 var file=FileAccess.open(base+"render_check.json",FileAccess.WRITE);file.store_string(JSON.stringify(log));file.close()
