...
func _process(delta: float) -> bool:
	...
	if battle.battle_phase == "active":
		...
		load("res://tools/slowdown_profile/runtime.gd").advance(battle, delta)
	...
	if elapsed >= 12.0:
		...
		active_samples.sort()
		report["timings"] = load("res://tools/slowdown_profile/probe.gd").stats
		...
		FileAccess.open(out_dir + "/profile_report.json", FileAccess.WRITE).store_string(JSON.stringify(report,"  "))
