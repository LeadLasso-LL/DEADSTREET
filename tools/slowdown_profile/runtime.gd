...
const BattleCombatBehaviorService := preload("res://tools/slowdown_profile/combat.gd")
...
static func _raw_advance(battle_state: BattleState, delta_seconds: float) -> BattleRuntimeResult:
...
static func _raw__finish_runtime(
...
static func advance(battle_state: BattleState, delta_seconds: float) -> BattleRuntimeResult:
	...
	Probe.record('runtime.advance', Time.get_ticks_usec()-t)
	return result
...
static func _finish_runtime(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__finish_runtime(battle_state, delta_seconds, elapsed_before, victory_result, resolved_this_pass)
	Probe.record('runtime._finish_runtime', Time.get_ticks_usec()-t)
	return result
