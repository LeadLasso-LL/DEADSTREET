...
static func _raw_check_participant_to_participant(
...
static func _raw_check_segment(
	battle_state: BattleState,
...
static func _raw__validate_geometry(
	battle_state: BattleState,
	source_participant_id: String,
	target_participant_id: String
) -> BattleLineOfSightResult:
...
static func _raw__is_eligible_endpoint(participant: BattleParticipant) -> bool:
	if participant == null:
		return false
...
static func _raw__trace_segment_cached(
	battle_state: BattleState,
	start_position: Vector2,
	end_position: Vector2,
	source_participant_id: String,
...
static func _raw__trace_segment(
...
static func _raw__interior_bounds(bounds: Rect2) -> Rect2:
	return Rect2(
...
static func check_participant_to_participant(
	...
	var result = _raw_check_participant_to_participant(battle_state, source_participant_id, target_participant_id)
	Probe.record('los.check_participant_to_participant', Time.get_ticks_usec()-t)
	return result
...
static func check_segment(
	...
	var t := Time.get_ticks_usec()
	var result = _raw_check_segment(battle_state, start_position, end_position)
	Probe.record('los.check_segment', Time.get_ticks_usec()-t)
	return result
...
static func _validate_geometry(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__validate_geometry(battle_state, source_participant_id, target_participant_id)
	Probe.record('los._validate_geometry', Time.get_ticks_usec()-t)
	return result


static func _is_eligible_endpoint(participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__is_eligible_endpoint(participant)
	Probe.record('los._is_eligible_endpoint', Time.get_ticks_usec()-t)
	return result
...
static func _trace_segment_cached(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__trace_segment_cached(battle_state, start_position, end_position, source_participant_id, target_participant_id)
	Probe.record('los._trace_segment_cached', Time.get_ticks_usec()-t)
	return result
...
static func _trace_segment(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__trace_segment(geometry, start_position, end_position, source_participant_id, target_participant_id)
	Probe.record('los._trace_segment', Time.get_ticks_usec()-t)
	return result


static func _interior_bounds(bounds: Rect2) -> Rect2:
	var t := Time.get_ticks_usec()
	var result = _raw__interior_bounds(bounds)
	Probe.record('los._interior_bounds', Time.get_ticks_usec()-t)
	return result
