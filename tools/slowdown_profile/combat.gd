extends RefCounted
const Probe = preload('res://tools/slowdown_profile/probe.gd')
...
const BattleLineOfSightService := preload("res://tools/slowdown_profile/los.gd")
...
const BattleCombatCoverEvaluationService := preload(
	"res://tools/slowdown_profile/cover_eval.gd"
)
...
static func _raw_advance(battle_state: BattleState, delta_seconds: float) -> BattleCombatBehaviorResult:
...
static func _raw__update_acquire_reaction(
...
static func _raw__clear_acquire_reaction(participant: BattleParticipant) -> void:
	if participant == null:
		return
	participant.acquire_reaction_remaining_seconds = 0.0
...
static func _raw__update_sniper_aim(
	battle_state: BattleState,
	participant: BattleParticipant
) -> void:
	if participant == null:
...
static func _raw__try_execute_shot(
	battle_state: BattleState,
	participant: BattleParticipant
) -> BattleAttackEvent:
	if participant == null:
...
static func _raw__update_defend_position_behavior(
	battle_state: BattleState,
	participant: BattleParticipant,
...
static func _raw__defend_can_fire(
	battle_state: BattleState,
	participant: BattleParticipant,
...
static func _raw__defend_los_blocks_fire(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> bool:
...
static func _raw__defend_keep_current_path(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> bool:
...
static func _raw__try_occupy_arrived_defend_cover(
	battle_state: BattleState,
	participant: BattleParticipant
) -> void:
...
static func _raw__pursue_defend_cover(
	battle_state: BattleState,
	participant: BattleParticipant,
...
static func _raw__navigate_to_defend_point(
	battle_state: BattleState,
	participant: BattleParticipant,
	destination: Vector2,
	target: BattleParticipant,
...
static func _raw__update_wounded_cover_behavior(
	battle_state: BattleState,
	participant: BattleParticipant,
	search_counts: Dictionary,
	delta_seconds: float
...
static func _raw__commit_wounded_cover_slot(
	battle_state: BattleState,
	participant: BattleParticipant,
...
static func _raw__update_healthy_role_cover_behavior(
	battle_state: BattleState,
	participant: BattleParticipant,
	search_counts: Dictionary,
	delta_seconds: float
...
static func _raw__short_range_closing_applies(
	battle_state: BattleState,
	participant: BattleParticipant,
...
static func _raw__short_range_still_needs_to_close(
	battle_state: BattleState,
	participant: BattleParticipant,
	weapon_type_id: String
) -> bool:
...
static func _raw__closing_command_permits(battle_state: BattleState, participant: BattleParticipant) -> bool:
	if participant == null:
		return false
...
static func _raw__is_too_far_for_preferred_band(
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
) -> bool:
...
static func _raw__closing_should_commit(
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
) -> bool:
...
static func _raw__update_short_range_closing_behavior(
	battle_state: BattleState,
	participant: BattleParticipant,
	weapon_type_id: String,
	search_counts: Dictionary,
...
static func _raw__continue_closing_from_occupied(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String,
...
static func _raw__pursue_closing_cover(
	battle_state: BattleState,
	participant: BattleParticipant,
	slot: BattleCoverSlot,
	search_counts: Dictionary
...
static func _raw__best_closing_cover_slot(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__healthy_keep_closing_cover_decision(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__cheap_closing_slot_still_valid(
	battle_state: BattleState,
	participant: BattleParticipant,
	slot: BattleCoverSlot,
	hostile: BattleParticipant,
...
static func _raw__closing_posture_cycle_in_progress(participant: BattleParticipant) -> bool:
	if participant == null:
		return false
...
static func _raw__begin_closing_occupancy_if_needed(participant: BattleParticipant) -> void:
	if participant == null:
		return
...
static func _raw__closing_occupied_slot_is_protective(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> bool:
...
static func _raw__vacate_owned_cover(battle_state: BattleState, participant: BattleParticipant) -> void:
	if participant == null:
		return
...
static func _raw__clear_closing_pursuit(battle_state: BattleState, participant: BattleParticipant) -> void:
	if participant == null:
		return
	if participant.combat_move_mode != MOVE_CLOSE:
		return
...
static func _raw__can_take_worthwhile_closing_shot(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> bool:
...
static func _raw__should_withhold_out_of_band_shot(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> bool:
...
static func _raw__should_continue_closing_after_shot(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
	if participant == null or participant.is_wounded:
...
static func _raw__pursue_healthy_role_cover(
	battle_state: BattleState,
	participant: BattleParticipant,
	slot: BattleCoverSlot,
	search_counts: Dictionary
...
static func _raw__healthy_current_target(
	battle_state: BattleState,
	participant: BattleParticipant
) -> BattleParticipant:
...
static func _raw__healthy_occupied_cover_should_persist(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__is_closing_occupancy(participant: BattleParticipant) -> bool:
	if participant == null:
		return false
	if participant.occupied_cover_slot_id.is_empty():
		return false
...
static func _raw__is_defender_side(battle_state: BattleState, participant: BattleParticipant) -> bool:
	if participant == null:
		return false
...
static func _raw__occupied_cover_is_unsafe_from_close_threat(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
	if participant == null or battle_state == null:
...
static func _raw__best_healthy_role_cover_slot(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__wounded_occupied_cover_is_suitable(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
	return BattleCombatCoverEvaluationService.occupied_cover_is_suitable(
...
static func _raw__living_count_on_side(battle_state: BattleState, side_id: String) -> int:
...
static func _raw__wounded_would_fire_if_exposed(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
	if participant == null:
...
static func _raw__wounded_should_break_stalemate(
	battle_state: BattleState,
	participant: BattleParticipant,
	delta_seconds: float
) -> bool:
...
static func _raw__wounded_break_stalemate(
	battle_state: BattleState,
	participant: BattleParticipant,
	search_counts: Dictionary
) -> String:
...
static func _raw__wounded_reenter_destination(
	battle_state: BattleState,
	participant: BattleParticipant,
	hostile: BattleParticipant
) -> Vector2:
...
static func _raw__wounded_keep_reenter_decision(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
	if participant == null:
...
static func _raw__navigate_to_combat_destination(
	battle_state: BattleState,
	participant: BattleParticipant,
	destination: Vector2,
	move_mode: String,
...
static func _raw__healthy_slot_is_role_suitable(
	slot: BattleCoverSlot,
	target: BattleParticipant,
	weapon_type_id: String
) -> bool:
...
static func _raw__increment_search(search_counts: Dictionary, key: String) -> void:
	if search_counts == null or key.is_empty():
		return
	search_counts[key] = int(search_counts.get(key, 0)) + 1


static func _raw__combat_context_id(context: BattleParticipant) -> String:
	if context == null:
		return ""
	return context.participant_id


static func _raw__combat_decision_key_for(
	battle_state: BattleState,
	participant: BattleParticipant,
	context_id: String
) -> String:
...
static func _raw__bind_combat_decision(
	battle_state: BattleState,
	participant: BattleParticipant,
	context_id: String
) -> void:
...
static func _raw__cover_availability_stamp(battle_state: BattleState) -> String:
...
static func _raw__healthy_no_role_cover_key(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__bind_no_role_cover_decision(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__clear_no_role_cover_decision(participant: BattleParticipant) -> void:
	if participant == null:
		return
...
static func _raw__healthy_keep_no_role_cover_decision(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__combat_decision_matches(
	battle_state: BattleState,
	participant: BattleParticipant,
	context_id: String
) -> bool:
...
static func _raw__reserved_cover_slot(
	battle_state: BattleState,
	participant: BattleParticipant
) -> BattleCoverSlot:
...
static func _raw__cheap_cover_decision_still_valid(
	battle_state: BattleState,
	participant: BattleParticipant,
	slot: BattleCoverSlot,
	hostile: BattleParticipant,
...
static func _raw__healthy_keep_current_cover_decision(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__wounded_keep_current_cover_decision(
	battle_state: BattleState,
	participant: BattleParticipant,
	hostile: BattleParticipant
) -> bool:
...
static func _raw__find_combat_navigation_path(
	battle_state: BattleState,
...
static func _raw__first_reservable_ranked_slot(
	battle_state: BattleState,
	participant: BattleParticipant,
...
static func _raw__active_hostile(
	battle_state: BattleState,
	participant: BattleParticipant
) -> BattleParticipant:
...
static func _raw__has_los_to_nearest_hostile(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
	var hostile: BattleParticipant = _active_hostile(battle_state, participant)
...
static func _raw__wounded_threat_override_without_cover(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
...
static func _raw__nearest_reachable_cover_any(
	battle_state: BattleState,
	participant: BattleParticipant
) -> BattleCoverSlot:
	if participant == null or battle_state == null or battle_state.battlefield_geometry == null:
...
static func _raw__nearest_cover_sort(left: Dictionary, right: Dictionary) -> bool:
...
static func _raw__participant_weapon_type_id(participant: BattleParticipant) -> String:
	if participant == null:
		return ""
...
static func _raw__reconcile_cover_state(battle_state: BattleState, participant: BattleParticipant) -> void:
	if participant == null or battle_state == null or battle_state.battlefield_geometry == null:
		return
...
static func _raw__update_occupied_cover_posture(
	battle_state: BattleState,
	participant: BattleParticipant,
	delta_seconds: float,
...
static func _raw__has_valid_occupancy(battle_state: BattleState, participant: BattleParticipant) -> bool:
	return BattleCoverService.occupancy_is_valid(battle_state, participant)


static func _raw__has_valid_reservation(battle_state: BattleState, participant: BattleParticipant) -> bool:
	if participant == null or battle_state == null or battle_state.battlefield_geometry == null:
		return false
...
static func _raw__ensure_cover_reservation(
	battle_state: BattleState,
	participant: BattleParticipant,
	slot: BattleCoverSlot
) -> bool:
...
static func _raw__release_owned_reservation(battle_state: BattleState, participant: BattleParticipant) -> void:
...
static func _raw__navigate_to_cover(
	battle_state: BattleState,
	participant: BattleParticipant,
	slot: BattleCoverSlot,
	move_mode: String,
...
static func _raw__should_keep_cover_path(
	participant: BattleParticipant,
	slot: BattleCoverSlot,
...
static func _raw__nearest_hostile_distance_sq(battle_state: BattleState, source: BattleParticipant) -> float:
...
static func _raw__is_eligible_threat(
	battle_state: BattleState,
...
static func _raw__update_combat_movement(
	battle_state: BattleState,
	participant: BattleParticipant,
	search_counts: Dictionary
) -> int:
...
static func _raw__participant_force_command_id(
	battle_state: BattleState,
	participant: BattleParticipant
) -> String:
	if battle_state == null or participant == null:
...
static func _raw__hold_suppresses_target_approach(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
	if participant == null:
...
static func _raw__healthy_role_requires_weapon_range(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	weapon_type_id: String
...
static func _raw__short_range_exposed_threat_urgency(
	battle_state: BattleState,
	participant: BattleParticipant,
	hostile: BattleParticipant,
	weapon_type_id: String
...
static func _raw__push_applies(battle_state: BattleState, participant: BattleParticipant) -> bool:
	if participant == null:
		return false
	if participant.is_wounded:
		return false
...
static func _raw__focus_applies(battle_state: BattleState, participant: BattleParticipant) -> bool:
	if participant == null:
		return false
	if participant.is_wounded:
		return false
...
static func _raw__fall_back_applies(battle_state: BattleState, participant: BattleParticipant) -> bool:
	if participant == null:
		return false
	if participant.is_wounded:
		return false
...
static func _raw__update_fall_back_movement(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	search_counts: Dictionary
...
static func _raw__should_keep_fall_back_path(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> bool:
...
static func _raw__fall_back_destination_result(found: bool, destination: Vector2 = Vector2.ZERO) -> Dictionary:
...
static func _raw__fall_back_destination(
	battle_state: BattleState,
	source: BattleParticipant,
	target: BattleParticipant
) -> Dictionary:
...
static func _raw__is_rearward_or_neutral_displacement(
...
static func _raw__focus_lateral_direction(
	battle_state: BattleState,
	participant: BattleParticipant
) -> Vector2:
	var command_id: String = _participant_force_command_id(battle_state, participant)
...
static func _raw__focus_biased_destination(
	battle_state: BattleState,
	source: BattleParticipant,
	target: BattleParticipant,
...
static func _raw__focus_range_is_in_preferred_band(
...
static func _raw__approach_movement_status(
	push_pressure: bool,
...
static func _raw__should_suppress_autonomous_aggressive_approach(
	battle_state: BattleState,
	participant: BattleParticipant,
	push_pressure: bool
) -> bool:
...
static func _raw__push_uses_controlled_advance(weapon_type_id: String) -> bool:
	return (
...
static func _raw__push_approach_destination(
	battle_state: BattleState,
	source: BattleParticipant,
	target: BattleParticipant
) -> Vector2:
...
static func _raw__preferred_band_stand_off(
	battle_state: BattleState,
	source: BattleParticipant,
	target: BattleParticipant,
...
static func _raw__desired_move_mode(
	battle_state: BattleState,
	source: BattleParticipant,
	target: BattleParticipant
) -> String:
...
static func _raw__retreat_destination(
	battle_state: BattleState,
	source: BattleParticipant,
	target: BattleParticipant
) -> Vector2:
...
static func _raw__should_keep_combat_path(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant,
	move_mode: String,
...
static func _raw__is_valid_navigation_destination(battle_state: BattleState, point: Vector2) -> bool:
...
static func _raw__halt_wounded_stand_motion(participant: BattleParticipant) -> void:
	_halt_stand_motion(participant)


static func _raw__halt_stand_motion(participant: BattleParticipant) -> void:
	if participant == null:
		return
...
static func _raw__anchor_to_occupied_slot(battle_state: BattleState, participant: BattleParticipant) -> void:
	if participant == null or battle_state == null or battle_state.battlefield_geometry == null:
		return
	if participant.occupied_cover_slot_id.is_empty():
		return
...
static func _raw__anchor_and_halt_occupied_cover(
	battle_state: BattleState,
	participant: BattleParticipant
) -> void:
	if participant == null:
...
static func _raw__complete_successful_cover_occupation(
	battle_state: BattleState,
	participant: BattleParticipant
) -> void:
	_anchor_and_halt_occupied_cover(battle_state, participant)
...
static func _raw__closing_staging_key(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> String:
...
static func _raw__bind_closing_staging_hold(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> void:
...
static func _raw__closing_staging_hold_is_current(
	battle_state: BattleState,
	participant: BattleParticipant,
	target: BattleParticipant
) -> bool:
...
static func _raw__has_external_navigation(participant: BattleParticipant) -> bool:
	if participant == null:
		return false
...
static func _raw__sync_player_tactical_intent(participant: BattleParticipant) -> void:
	if participant == null:
		return
...
static func _raw__update_player_cover_behavior(
	battle_state: BattleState,
	participant: BattleParticipant,
	search_counts: Dictionary
) -> String:
...
static func _raw__player_cover_object_is_usable(
	battle_state: BattleState,
	participant: BattleParticipant
) -> bool:
	if battle_state == null or participant == null or battle_state.battlefield_geometry == null:
...
static func _raw__player_cover_occupies_commanded_object(
	battle_state: BattleState,
	participant: BattleParticipant,
...
static func _raw__player_cover_current_slot(
	battle_state: BattleState,
	participant: BattleParticipant
) -> BattleCoverSlot:
	if battle_state == null or participant == null or battle_state.battlefield_geometry == null:
...
static func _raw__player_cover_resolve_same_object_slot(
	battle_state: BattleState,
	participant: BattleParticipant,
	cover_object_id: String
) -> BattleCoverSlot:
...
static func _raw__clear_owned_combat_navigation(participant: BattleParticipant) -> void:
	if participant == null:
		return
...
static func _raw__ensure_combat_movement_speed(participant: BattleParticipant) -> void:
	if participant == null:
		return
	if participant.is_wounded:
...
static func _raw__ensure_wounded_combat_movement_speed(participant: BattleParticipant) -> void:
	if participant == null:
		return
...
static func _raw__is_positioned(participant: BattleParticipant) -> bool:
	if participant == null:
		return false
...
static func _raw__is_finite_vector(value: Vector2) -> bool:
	return is_finite(value.x) and is_finite(value.y)


static func _raw__sorted_participant_ids(battle_state: BattleState) -> Array[String]:
...
static func advance(battle_state: BattleState, delta_seconds: float) -> BattleCombatBehaviorResult:
	...
	var result = _raw_advance(battle_state, delta_seconds)
	Probe.record('combat.advance', Time.get_ticks_usec()-t)
	return result
...
static func _update_acquire_reaction(
	...
	var t := Time.get_ticks_usec()
	_raw__update_acquire_reaction(battle_state, participant)
	Probe.record('combat._update_acquire_reaction', Time.get_ticks_usec()-t)


static func _clear_acquire_reaction(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__clear_acquire_reaction(participant)
	Probe.record('combat._clear_acquire_reaction', Time.get_ticks_usec()-t)
...
static func _update_sniper_aim(
	...
	var t := Time.get_ticks_usec()
	_raw__update_sniper_aim(battle_state, participant)
	Probe.record('combat._update_sniper_aim', Time.get_ticks_usec()-t)
...
static func _try_execute_shot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__try_execute_shot(battle_state, participant)
	Probe.record('combat._try_execute_shot', Time.get_ticks_usec()-t)
	return result
...
static func _update_defend_position_behavior(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__update_defend_position_behavior(battle_state, participant, search_counts)
	Probe.record('combat._update_defend_position_behavior', Time.get_ticks_usec()-t)
	return result
...
static func _defend_can_fire(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__defend_can_fire(battle_state, participant, target)
	Probe.record('combat._defend_can_fire', Time.get_ticks_usec()-t)
	return result
...
static func _defend_los_blocks_fire(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__defend_los_blocks_fire(battle_state, participant, target)
	Probe.record('combat._defend_los_blocks_fire', Time.get_ticks_usec()-t)
	return result
...
static func _defend_keep_current_path(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__defend_keep_current_path(battle_state, participant, target)
	Probe.record('combat._defend_keep_current_path', Time.get_ticks_usec()-t)
	return result
...
static func _try_occupy_arrived_defend_cover(
	...
	var t := Time.get_ticks_usec()
	_raw__try_occupy_arrived_defend_cover(battle_state, participant)
	Probe.record('combat._try_occupy_arrived_defend_cover', Time.get_ticks_usec()-t)
...
static func _pursue_defend_cover(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__pursue_defend_cover(battle_state, participant, slot, search_counts)
	Probe.record('combat._pursue_defend_cover', Time.get_ticks_usec()-t)
	return result
...
static func _navigate_to_defend_point(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__navigate_to_defend_point(battle_state, participant, destination, target, search_counts)
	Probe.record('combat._navigate_to_defend_point', Time.get_ticks_usec()-t)
	return result
...
static func _update_wounded_cover_behavior(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__update_wounded_cover_behavior(battle_state, participant, search_counts, delta_seconds)
	Probe.record('combat._update_wounded_cover_behavior', Time.get_ticks_usec()-t)
	return result
...
static func _commit_wounded_cover_slot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__commit_wounded_cover_slot(battle_state, participant, cover_slot, hostile, search_counts)
	Probe.record('combat._commit_wounded_cover_slot', Time.get_ticks_usec()-t)
	return result
...
static func _update_healthy_role_cover_behavior(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__update_healthy_role_cover_behavior(battle_state, participant, search_counts, delta_seconds)
	Probe.record('combat._update_healthy_role_cover_behavior', Time.get_ticks_usec()-t)
	return result
...
static func _short_range_closing_applies(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__short_range_closing_applies(battle_state, participant, weapon_type_id)
	Probe.record('combat._short_range_closing_applies', Time.get_ticks_usec()-t)
	return result
...
static func _short_range_still_needs_to_close(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__short_range_still_needs_to_close(battle_state, participant, weapon_type_id)
	Probe.record('combat._short_range_still_needs_to_close', Time.get_ticks_usec()-t)
	return result


static func _closing_command_permits(battle_state: BattleState, participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__closing_command_permits(battle_state, participant)
	Probe.record('combat._closing_command_permits', Time.get_ticks_usec()-t)
	return result
...
static func _is_too_far_for_preferred_band(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__is_too_far_for_preferred_band(participant, target, weapon_type_id)
	Probe.record('combat._is_too_far_for_preferred_band', Time.get_ticks_usec()-t)
	return result
...
static func _closing_should_commit(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__closing_should_commit(participant, target, weapon_type_id)
	Probe.record('combat._closing_should_commit', Time.get_ticks_usec()-t)
	return result
...
static func _update_short_range_closing_behavior(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__update_short_range_closing_behavior(battle_state, participant, weapon_type_id, search_counts, delta_seconds)
	Probe.record('combat._update_short_range_closing_behavior', Time.get_ticks_usec()-t)
	return result
...
static func _continue_closing_from_occupied(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__continue_closing_from_occupied(battle_state, participant, target, weapon_type_id, search_counts)
	Probe.record('combat._continue_closing_from_occupied', Time.get_ticks_usec()-t)
	return result
...
static func _pursue_closing_cover(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__pursue_closing_cover(battle_state, participant, slot, search_counts)
	Probe.record('combat._pursue_closing_cover', Time.get_ticks_usec()-t)
	return result
...
static func _best_closing_cover_slot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__best_closing_cover_slot(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._best_closing_cover_slot', Time.get_ticks_usec()-t)
	return result
...
static func _healthy_keep_closing_cover_decision(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__healthy_keep_closing_cover_decision(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._healthy_keep_closing_cover_decision', Time.get_ticks_usec()-t)
	return result
...
static func _cheap_closing_slot_still_valid(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__cheap_closing_slot_still_valid(battle_state, participant, slot, hostile, weapon_type_id)
	Probe.record('combat._cheap_closing_slot_still_valid', Time.get_ticks_usec()-t)
	return result


static func _closing_posture_cycle_in_progress(participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__closing_posture_cycle_in_progress(participant)
	Probe.record('combat._closing_posture_cycle_in_progress', Time.get_ticks_usec()-t)
	return result


static func _begin_closing_occupancy_if_needed(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__begin_closing_occupancy_if_needed(participant)
	Probe.record('combat._begin_closing_occupancy_if_needed', Time.get_ticks_usec()-t)
...
static func _closing_occupied_slot_is_protective(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__closing_occupied_slot_is_protective(battle_state, participant, target)
	Probe.record('combat._closing_occupied_slot_is_protective', Time.get_ticks_usec()-t)
	return result


static func _vacate_owned_cover(battle_state: BattleState, participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__vacate_owned_cover(battle_state, participant)
	Probe.record('combat._vacate_owned_cover', Time.get_ticks_usec()-t)


static func _clear_closing_pursuit(battle_state: BattleState, participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__clear_closing_pursuit(battle_state, participant)
	Probe.record('combat._clear_closing_pursuit', Time.get_ticks_usec()-t)
...
static func _can_take_worthwhile_closing_shot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__can_take_worthwhile_closing_shot(battle_state, participant, target)
	Probe.record('combat._can_take_worthwhile_closing_shot', Time.get_ticks_usec()-t)
	return result
...
static func _should_withhold_out_of_band_shot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__should_withhold_out_of_band_shot(battle_state, participant, target)
	Probe.record('combat._should_withhold_out_of_band_shot', Time.get_ticks_usec()-t)
	return result
...
static func _should_continue_closing_after_shot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__should_continue_closing_after_shot(battle_state, participant)
	Probe.record('combat._should_continue_closing_after_shot', Time.get_ticks_usec()-t)
	return result
...
static func _pursue_healthy_role_cover(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__pursue_healthy_role_cover(battle_state, participant, slot, search_counts)
	Probe.record('combat._pursue_healthy_role_cover', Time.get_ticks_usec()-t)
	return result
...
static func _healthy_current_target(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__healthy_current_target(battle_state, participant)
	Probe.record('combat._healthy_current_target', Time.get_ticks_usec()-t)
	return result
...
static func _healthy_occupied_cover_should_persist(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__healthy_occupied_cover_should_persist(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._healthy_occupied_cover_should_persist', Time.get_ticks_usec()-t)
	return result


static func _is_closing_occupancy(participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__is_closing_occupancy(participant)
	Probe.record('combat._is_closing_occupancy', Time.get_ticks_usec()-t)
	return result


static func _is_defender_side(battle_state: BattleState, participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__is_defender_side(battle_state, participant)
	Probe.record('combat._is_defender_side', Time.get_ticks_usec()-t)
	return result
...
static func _occupied_cover_is_unsafe_from_close_threat(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__occupied_cover_is_unsafe_from_close_threat(battle_state, participant)
	Probe.record('combat._occupied_cover_is_unsafe_from_close_threat', Time.get_ticks_usec()-t)
	return result
...
static func _best_healthy_role_cover_slot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__best_healthy_role_cover_slot(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._best_healthy_role_cover_slot', Time.get_ticks_usec()-t)
	return result
...
static func _wounded_occupied_cover_is_suitable(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__wounded_occupied_cover_is_suitable(battle_state, participant)
	Probe.record('combat._wounded_occupied_cover_is_suitable', Time.get_ticks_usec()-t)
	return result


static func _living_count_on_side(battle_state: BattleState, side_id: String) -> int:
	var t := Time.get_ticks_usec()
	var result = _raw__living_count_on_side(battle_state, side_id)
	Probe.record('combat._living_count_on_side', Time.get_ticks_usec()-t)
	return result
...
static func _wounded_would_fire_if_exposed(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__wounded_would_fire_if_exposed(battle_state, participant)
	Probe.record('combat._wounded_would_fire_if_exposed', Time.get_ticks_usec()-t)
	return result
...
static func _wounded_should_break_stalemate(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__wounded_should_break_stalemate(battle_state, participant, delta_seconds)
	Probe.record('combat._wounded_should_break_stalemate', Time.get_ticks_usec()-t)
	return result
...
static func _wounded_break_stalemate(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__wounded_break_stalemate(battle_state, participant, search_counts)
	Probe.record('combat._wounded_break_stalemate', Time.get_ticks_usec()-t)
	return result
...
static func _wounded_reenter_destination(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__wounded_reenter_destination(battle_state, participant, hostile)
	Probe.record('combat._wounded_reenter_destination', Time.get_ticks_usec()-t)
	return result
...
static func _wounded_keep_reenter_decision(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__wounded_keep_reenter_decision(battle_state, participant)
	Probe.record('combat._wounded_keep_reenter_decision', Time.get_ticks_usec()-t)
	return result
...
static func _navigate_to_combat_destination(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__navigate_to_combat_destination(battle_state, participant, destination, move_mode, context_id, search_counts)
	Probe.record('combat._navigate_to_combat_destination', Time.get_ticks_usec()-t)
	return result
...
static func _healthy_slot_is_role_suitable(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__healthy_slot_is_role_suitable(slot, target, weapon_type_id)
	Probe.record('combat._healthy_slot_is_role_suitable', Time.get_ticks_usec()-t)
	return result


static func _increment_search(search_counts: Dictionary, key: String) -> void:
	var t := Time.get_ticks_usec()
	_raw__increment_search(search_counts, key)
	Probe.record('combat._increment_search', Time.get_ticks_usec()-t)


static func _combat_context_id(context: BattleParticipant) -> String:
	var t := Time.get_ticks_usec()
	var result = _raw__combat_context_id(context)
	Probe.record('combat._combat_context_id', Time.get_ticks_usec()-t)
	return result
...
static func _combat_decision_key_for(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__combat_decision_key_for(battle_state, participant, context_id)
	Probe.record('combat._combat_decision_key_for', Time.get_ticks_usec()-t)
	return result
...
static func _bind_combat_decision(
	...
	var t := Time.get_ticks_usec()
	_raw__bind_combat_decision(battle_state, participant, context_id)
	Probe.record('combat._bind_combat_decision', Time.get_ticks_usec()-t)


static func _cover_availability_stamp(battle_state: BattleState) -> String:
	var t := Time.get_ticks_usec()
	var result = _raw__cover_availability_stamp(battle_state)
	Probe.record('combat._cover_availability_stamp', Time.get_ticks_usec()-t)
	return result
...
static func _healthy_no_role_cover_key(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__healthy_no_role_cover_key(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._healthy_no_role_cover_key', Time.get_ticks_usec()-t)
	return result
...
static func _bind_no_role_cover_decision(
	...
	var t := Time.get_ticks_usec()
	_raw__bind_no_role_cover_decision(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._bind_no_role_cover_decision', Time.get_ticks_usec()-t)


static func _clear_no_role_cover_decision(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__clear_no_role_cover_decision(participant)
	Probe.record('combat._clear_no_role_cover_decision', Time.get_ticks_usec()-t)
...
static func _healthy_keep_no_role_cover_decision(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__healthy_keep_no_role_cover_decision(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._healthy_keep_no_role_cover_decision', Time.get_ticks_usec()-t)
	return result
...
static func _combat_decision_matches(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__combat_decision_matches(battle_state, participant, context_id)
	Probe.record('combat._combat_decision_matches', Time.get_ticks_usec()-t)
	return result
...
static func _reserved_cover_slot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__reserved_cover_slot(battle_state, participant)
	Probe.record('combat._reserved_cover_slot', Time.get_ticks_usec()-t)
	return result
...
static func _cheap_cover_decision_still_valid(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__cheap_cover_decision_still_valid(battle_state, participant, slot, hostile, require_weapon_range, weapon_type_id, require_role, max_move_distance)
	Probe.record('combat._cheap_cover_decision_still_valid', Time.get_ticks_usec()-t)
	return result
...
static func _healthy_keep_current_cover_decision(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__healthy_keep_current_cover_decision(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._healthy_keep_current_cover_decision', Time.get_ticks_usec()-t)
	return result
...
static func _wounded_keep_current_cover_decision(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__wounded_keep_current_cover_decision(battle_state, participant, hostile)
	Probe.record('combat._wounded_keep_current_cover_decision', Time.get_ticks_usec()-t)
	return result
...
static func _find_combat_navigation_path(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__find_combat_navigation_path(battle_state, from_position, destination, search_counts)
	Probe.record('combat._find_combat_navigation_path', Time.get_ticks_usec()-t)
	return result
...
static func _first_reservable_ranked_slot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__first_reservable_ranked_slot(battle_state, participant, ranked, exclude_slot_id)
	Probe.record('combat._first_reservable_ranked_slot', Time.get_ticks_usec()-t)
	return result
...
static func _active_hostile(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__active_hostile(battle_state, participant)
	Probe.record('combat._active_hostile', Time.get_ticks_usec()-t)
	return result
...
static func _has_los_to_nearest_hostile(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__has_los_to_nearest_hostile(battle_state, participant)
	Probe.record('combat._has_los_to_nearest_hostile', Time.get_ticks_usec()-t)
	return result
...
static func _wounded_threat_override_without_cover(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__wounded_threat_override_without_cover(battle_state, participant)
	Probe.record('combat._wounded_threat_override_without_cover', Time.get_ticks_usec()-t)
	return result
...
static func _nearest_reachable_cover_any(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__nearest_reachable_cover_any(battle_state, participant)
	Probe.record('combat._nearest_reachable_cover_any', Time.get_ticks_usec()-t)
	return result


static func _nearest_cover_sort(left: Dictionary, right: Dictionary) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__nearest_cover_sort(left, right)
	Probe.record('combat._nearest_cover_sort', Time.get_ticks_usec()-t)
	return result


static func _participant_weapon_type_id(participant: BattleParticipant) -> String:
	var t := Time.get_ticks_usec()
	var result = _raw__participant_weapon_type_id(participant)
	Probe.record('combat._participant_weapon_type_id', Time.get_ticks_usec()-t)
	return result


static func _reconcile_cover_state(battle_state: BattleState, participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__reconcile_cover_state(battle_state, participant)
	Probe.record('combat._reconcile_cover_state', Time.get_ticks_usec()-t)
...
static func _update_occupied_cover_posture(
	...
	var t := Time.get_ticks_usec()
	_raw__update_occupied_cover_posture(battle_state, participant, delta_seconds, allow_begin_expose)
	Probe.record('combat._update_occupied_cover_posture', Time.get_ticks_usec()-t)


static func _has_valid_occupancy(battle_state: BattleState, participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__has_valid_occupancy(battle_state, participant)
	Probe.record('combat._has_valid_occupancy', Time.get_ticks_usec()-t)
	return result


static func _has_valid_reservation(battle_state: BattleState, participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__has_valid_reservation(battle_state, participant)
	Probe.record('combat._has_valid_reservation', Time.get_ticks_usec()-t)
	return result
...
static func _ensure_cover_reservation(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__ensure_cover_reservation(battle_state, participant, slot)
	Probe.record('combat._ensure_cover_reservation', Time.get_ticks_usec()-t)
	return result


static func _release_owned_reservation(battle_state: BattleState, participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__release_owned_reservation(battle_state, participant)
	Probe.record('combat._release_owned_reservation', Time.get_ticks_usec()-t)
...
static func _navigate_to_cover(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__navigate_to_cover(battle_state, participant, slot, move_mode, context_id, search_counts)
	Probe.record('combat._navigate_to_cover', Time.get_ticks_usec()-t)
	return result
...
static func _should_keep_cover_path(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__should_keep_cover_path(participant, slot, move_mode)
	Probe.record('combat._should_keep_cover_path', Time.get_ticks_usec()-t)
	return result


static func _nearest_hostile_distance_sq(battle_state: BattleState, source: BattleParticipant) -> float:
	var t := Time.get_ticks_usec()
	var result = _raw__nearest_hostile_distance_sq(battle_state, source)
	Probe.record('combat._nearest_hostile_distance_sq', Time.get_ticks_usec()-t)
	return result
...
static func _is_eligible_threat(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__is_eligible_threat(battle_state, source, candidate)
	Probe.record('combat._is_eligible_threat', Time.get_ticks_usec()-t)
	return result
...
static func _update_combat_movement(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__update_combat_movement(battle_state, participant, search_counts)
	Probe.record('combat._update_combat_movement', Time.get_ticks_usec()-t)
	return result
...
static func _participant_force_command_id(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__participant_force_command_id(battle_state, participant)
	Probe.record('combat._participant_force_command_id', Time.get_ticks_usec()-t)
	return result
...
static func _hold_suppresses_target_approach(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__hold_suppresses_target_approach(battle_state, participant)
	Probe.record('combat._hold_suppresses_target_approach', Time.get_ticks_usec()-t)
	return result
...
static func _healthy_role_requires_weapon_range(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__healthy_role_requires_weapon_range(battle_state, participant, target, weapon_type_id)
	Probe.record('combat._healthy_role_requires_weapon_range', Time.get_ticks_usec()-t)
	return result
...
static func _short_range_exposed_threat_urgency(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__short_range_exposed_threat_urgency(battle_state, participant, hostile, weapon_type_id)
	Probe.record('combat._short_range_exposed_threat_urgency', Time.get_ticks_usec()-t)
	return result


static func _push_applies(battle_state: BattleState, participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__push_applies(battle_state, participant)
	Probe.record('combat._push_applies', Time.get_ticks_usec()-t)
	return result


static func _focus_applies(battle_state: BattleState, participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__focus_applies(battle_state, participant)
	Probe.record('combat._focus_applies', Time.get_ticks_usec()-t)
	return result


static func _fall_back_applies(battle_state: BattleState, participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__fall_back_applies(battle_state, participant)
	Probe.record('combat._fall_back_applies', Time.get_ticks_usec()-t)
	return result
...
static func _update_fall_back_movement(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__update_fall_back_movement(battle_state, participant, target, search_counts)
	Probe.record('combat._update_fall_back_movement', Time.get_ticks_usec()-t)
	return result
...
static func _should_keep_fall_back_path(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__should_keep_fall_back_path(battle_state, participant, target)
	Probe.record('combat._should_keep_fall_back_path', Time.get_ticks_usec()-t)
	return result


static func _fall_back_destination_result(found: bool, destination: Vector2 = Vector2.ZERO) -> Dictionary:
	var t := Time.get_ticks_usec()
	var result = _raw__fall_back_destination_result(found, destination)
	Probe.record('combat._fall_back_destination_result', Time.get_ticks_usec()-t)
	return result
...
static func _fall_back_destination(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__fall_back_destination(battle_state, source, target)
	Probe.record('combat._fall_back_destination', Time.get_ticks_usec()-t)
	return result
...
static func _is_rearward_or_neutral_displacement(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__is_rearward_or_neutral_displacement(current_position, destination, forward)
	Probe.record('combat._is_rearward_or_neutral_displacement', Time.get_ticks_usec()-t)
	return result
...
static func _focus_lateral_direction(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__focus_lateral_direction(battle_state, participant)
	Probe.record('combat._focus_lateral_direction', Time.get_ticks_usec()-t)
	return result
...
static func _focus_biased_destination(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__focus_biased_destination(battle_state, source, target, base_destination)
	Probe.record('combat._focus_biased_destination', Time.get_ticks_usec()-t)
	return result
...
static func _focus_range_is_in_preferred_band(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__focus_range_is_in_preferred_band(range_distance, profile)
	Probe.record('combat._focus_range_is_in_preferred_band', Time.get_ticks_usec()-t)
	return result
...
static func _approach_movement_status(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__approach_movement_status(push_pressure, focus_bias_applied, focus_left)
	Probe.record('combat._approach_movement_status', Time.get_ticks_usec()-t)
	return result
...
static func _should_suppress_autonomous_aggressive_approach(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__should_suppress_autonomous_aggressive_approach(battle_state, participant, push_pressure)
	Probe.record('combat._should_suppress_autonomous_aggressive_approach', Time.get_ticks_usec()-t)
	return result


static func _push_uses_controlled_advance(weapon_type_id: String) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__push_uses_controlled_advance(weapon_type_id)
	Probe.record('combat._push_uses_controlled_advance', Time.get_ticks_usec()-t)
	return result
...
static func _push_approach_destination(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__push_approach_destination(battle_state, source, target)
	Probe.record('combat._push_approach_destination', Time.get_ticks_usec()-t)
	return result
...
static func _preferred_band_stand_off(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__preferred_band_stand_off(battle_state, source, target, preferred_min_distance, stand_off_distance)
	Probe.record('combat._preferred_band_stand_off', Time.get_ticks_usec()-t)
	return result
...
static func _desired_move_mode(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__desired_move_mode(battle_state, source, target)
	Probe.record('combat._desired_move_mode', Time.get_ticks_usec()-t)
	return result
...
static func _retreat_destination(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__retreat_destination(battle_state, source, target)
	Probe.record('combat._retreat_destination', Time.get_ticks_usec()-t)
	return result
...
static func _should_keep_combat_path(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__should_keep_combat_path(battle_state, participant, target, move_mode, destination)
	Probe.record('combat._should_keep_combat_path', Time.get_ticks_usec()-t)
	return result


static func _is_valid_navigation_destination(battle_state: BattleState, point: Vector2) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__is_valid_navigation_destination(battle_state, point)
	Probe.record('combat._is_valid_navigation_destination', Time.get_ticks_usec()-t)
	return result


static func _halt_wounded_stand_motion(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__halt_wounded_stand_motion(participant)
	Probe.record('combat._halt_wounded_stand_motion', Time.get_ticks_usec()-t)


static func _halt_stand_motion(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__halt_stand_motion(participant)
	Probe.record('combat._halt_stand_motion', Time.get_ticks_usec()-t)


static func _anchor_to_occupied_slot(battle_state: BattleState, participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__anchor_to_occupied_slot(battle_state, participant)
	Probe.record('combat._anchor_to_occupied_slot', Time.get_ticks_usec()-t)
...
static func _anchor_and_halt_occupied_cover(
	...
	var t := Time.get_ticks_usec()
	_raw__anchor_and_halt_occupied_cover(battle_state, participant)
	Probe.record('combat._anchor_and_halt_occupied_cover', Time.get_ticks_usec()-t)
...
static func _complete_successful_cover_occupation(
	...
	var t := Time.get_ticks_usec()
	_raw__complete_successful_cover_occupation(battle_state, participant)
	Probe.record('combat._complete_successful_cover_occupation', Time.get_ticks_usec()-t)
...
static func _closing_staging_key(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__closing_staging_key(battle_state, participant, target)
	Probe.record('combat._closing_staging_key', Time.get_ticks_usec()-t)
	return result
...
static func _bind_closing_staging_hold(
	...
	var t := Time.get_ticks_usec()
	_raw__bind_closing_staging_hold(battle_state, participant, target)
	Probe.record('combat._bind_closing_staging_hold', Time.get_ticks_usec()-t)
...
static func _closing_staging_hold_is_current(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__closing_staging_hold_is_current(battle_state, participant, target)
	Probe.record('combat._closing_staging_hold_is_current', Time.get_ticks_usec()-t)
	return result


static func _has_external_navigation(participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__has_external_navigation(participant)
	Probe.record('combat._has_external_navigation', Time.get_ticks_usec()-t)
	return result


static func _sync_player_tactical_intent(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__sync_player_tactical_intent(participant)
	Probe.record('combat._sync_player_tactical_intent', Time.get_ticks_usec()-t)
...
static func _update_player_cover_behavior(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__update_player_cover_behavior(battle_state, participant, search_counts)
	Probe.record('combat._update_player_cover_behavior', Time.get_ticks_usec()-t)
	return result
...
static func _player_cover_object_is_usable(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__player_cover_object_is_usable(battle_state, participant)
	Probe.record('combat._player_cover_object_is_usable', Time.get_ticks_usec()-t)
	return result
...
static func _player_cover_occupies_commanded_object(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__player_cover_occupies_commanded_object(battle_state, participant, cover_object_id)
	Probe.record('combat._player_cover_occupies_commanded_object', Time.get_ticks_usec()-t)
	return result
...
static func _player_cover_current_slot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__player_cover_current_slot(battle_state, participant)
	Probe.record('combat._player_cover_current_slot', Time.get_ticks_usec()-t)
	return result
...
static func _player_cover_resolve_same_object_slot(
	...
	var t := Time.get_ticks_usec()
	var result = _raw__player_cover_resolve_same_object_slot(battle_state, participant, cover_object_id)
	Probe.record('combat._player_cover_resolve_same_object_slot', Time.get_ticks_usec()-t)
	return result


static func _clear_owned_combat_navigation(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__clear_owned_combat_navigation(participant)
	Probe.record('combat._clear_owned_combat_navigation', Time.get_ticks_usec()-t)


static func _ensure_combat_movement_speed(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__ensure_combat_movement_speed(participant)
	Probe.record('combat._ensure_combat_movement_speed', Time.get_ticks_usec()-t)


static func _ensure_wounded_combat_movement_speed(participant: BattleParticipant) -> void:
	var t := Time.get_ticks_usec()
	_raw__ensure_wounded_combat_movement_speed(participant)
	Probe.record('combat._ensure_wounded_combat_movement_speed', Time.get_ticks_usec()-t)


static func _is_positioned(participant: BattleParticipant) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__is_positioned(participant)
	Probe.record('combat._is_positioned', Time.get_ticks_usec()-t)
	return result


static func _is_finite_vector(value: Vector2) -> bool:
	var t := Time.get_ticks_usec()
	var result = _raw__is_finite_vector(value)
	Probe.record('combat._is_finite_vector', Time.get_ticks_usec()-t)
	return result


static func _sorted_participant_ids(battle_state: BattleState) -> Array[String]:
	var t := Time.get_ticks_usec()
	var result = _raw__sorted_participant_ids(battle_state)
	Probe.record('combat._sorted_participant_ids', Time.get_ticks_usec()-t)
	return result
