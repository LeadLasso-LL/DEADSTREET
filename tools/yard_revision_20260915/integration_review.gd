...
func run():
 ...
 await RenderingServer.frame_post_draw;root.get_texture().get_image().save_png("res://tools/yard_revision_20260915/preset.png")
 ...
 for map_id in ["doble_ocho","harold","river_bridge","whittaker_estate"]:
  ...
  if scene.battle!=null:
   ...
   var mixer=view.battle_presentation.convoy_audio
   mixer.sync(b,{},true,0.,0.,0.,0.)
   for player in mixer.sources.values():
    if not player.get_meta("is_radio",false):continue
    var boost=linear_to_db(1.10) if map_id=="doble_ocho" and player.get_meta("side_id")==b.attacker_side_id and not player.has_meta("fixed_position") else 0.
    check(absf(player.volume_db-float(player.get_meta("base_gain"))-boost)<.001,map_id+" exact arrival radio level")
   ...
   for player in mixer.sources.values():
    if player.get_meta("is_radio",false):check(absf(player.volume_db-(float(player.get_meta("base_gain"))-float(player.get_meta("battle_drop",14.))))<.001,map_id+" unchanged combat radio level")
 ...
 FileAccess.open("res://tools/yard_revision_20260915/integration_review.json",FileAccess.WRITE).store_string(JSON.stringify(report,"\t"))
