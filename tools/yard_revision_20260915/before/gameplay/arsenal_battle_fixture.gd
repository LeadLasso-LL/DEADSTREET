...
static func setup_flexible(runtime: Node,loadouts: Dictionary,seed_value: int) -> Dictionary:
 ...
 if not preload("res://gameplay/blockade_battle_setup.gd").apply(b,loadouts.get("blockade_encounter",{})):return {"error":"invalid checkpoint battle geometry"}
 var controller=runtime.tactical_deployment_controller
 if not bridge and not estate and not yard:controller.choose_arrival("far")
 ...
 var covered=false if yard else (preload("res://gameplay/estate_battle_setup.gd").deploy_attackers(b) if estate else controller.place_unplaced_in_cover())
 ...
 runtime._sync_armor_store()
 runtime.set_process(false)
 var view=runtime.get_node("TacticalBattleView");view._dusk_zoom=1.60 if yard else (1.2 if estate else (2.4 if bridge else 1.30));view._dusk_pan=Vector2(38,-55) if yard else (Vector2(0,-42) if estate else (Vector2(-360,-22) if bridge else Vector2(-5,-30)));view._frame_camera()
