...
func fixed_defenders() -> bool:return is_bridge() or is_estate() or is_yard()
func begin_arrival():
...
class Context extends Control:
 ...
 func _draw():
  ...
  draw_string(p.font,Vector2(12,17).lerp(Vector2(30,29),t),("SOUTH SIDE  /  DOBLE OCHO AUTO YARD" if p.is_yard() else "CITY OUTSKIRTS  /  WHITTAKER ESTATE" if p.is_estate() else ("RIVER CROSSING  /  SUSPENSION BRIDGE" if p.is_bridge() else "MERCER HEIGHTS  /  HAROLD AVE.")),HORIZONTAL_ALIGNMENT_LEFT,-1,int(lerpf(10,14,t)),muted)
  ...
  var location="GARAGE RAID" if p.is_yard() else "ESTATE ASSAULT" if p.is_estate() else ("ROAD BLOCKADE" if p.is_bridge() else "HAROLD APARTMENTS")
