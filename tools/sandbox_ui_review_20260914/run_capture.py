...
files['tools/sandbox_ui_review_20260914/walkthrough.gd'] = str(out/'walkthrough.gd')
boot = '''extends Node
func _ready():
 if not ProjectSettings.load_resource_pack(DATA_PACK,false):get_tree().quit(5);return
 var tree=get_tree()
 tree.set_script(load("res://tools/sandbox_ui_review_20260914/walkthrough.gd"))
...
args=[str(stage/'godot.exe'),'--resolution','1152x860','--position','10,10','--fixed-fps','30','--disable-vsync']
if mode=='record':
    raw=out/'sandbox_ui_raw.avi'
...
with (out/(mode+'.log')).open('wb') as log:
    ...
    while process.poll() is None:
        ...
        if 'SCRIPT ERROR' in data or 'UI_REVIEW_FAIL' in data or time.monotonic()-start>780:
            process.kill();process.wait()
            raise RuntimeError(data[-9000:])
assert process.returncode==0,process.returncode
report=json.loads((out/(mode+'.json')).read_text())
assert report['checks']>100 and not report['errors'] and report['no_combat']
print('UI_CAPTURE_COMPLETE',mode,report['checks'],report['frames'],flush=True)
