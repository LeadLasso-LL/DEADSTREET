...
const Runtime=preload("res://tools/bridge_map/v3_runtime_probe.gd")
...
func run():
 ...
 costs.sort();report["runtime_mean_ms"]=total/costs.size();report["runtime_p95_ms"]=costs[int(costs.size()*.95)-1];report["runtime_max_ms"]=costs.back();report["active"]=stats()
 report["stage_totals_ms"]=Runtime.timings;report["stage_counts"]=Runtime.timing_counts
 report["combat_functions_ms"]=Runtime.BattleCombatBehaviorService.metrics;report["combat_functions_calls"]=Runtime.BattleCombatBehaviorService.counts
 DirAccess.make_dir_recursive_absolute("res://tools/bridge_map/v3_results")
 FileAccess.open("res://tools/bridge_map/v3_results/profile_before.json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "));print("BRIDGE_PROFILE ",report);quit()
