...
func run():
 ...
 var angled=0;var open=0
 for row in C.props():
  if row[2]!="traffic":continue
  if not is_zero_approx(row[4].y):angled+=1
  if row.size()>5 and row[5]>0:open+=1
  for box in C.traffic_footprints(row):check(box.has_area(),"positive traffic footprint")
 check(angled==5 and open==39,"five angled civilians, all 39 have open doors")
 for heading in [Vector2.RIGHT,Vector2(1,1).normalized(),Vector2.DOWN,Vector2(-1,1).normalized()]:
  var vehicle=load("res://battle/core/battle_vehicle.gd").new("probe","probe","f","attacker","bayou")
  vehicle.has_battle_position=true;vehicle.battle_position=Vector2(30,20);vehicle.set_facing_direction(heading)
  for spec in load("res://battle/vehicles/battle_arrival_service.gd").door_specs(vehicle):
   check(not Body.pose_intersects_rect(vehicle.battle_position,heading,Body.profile_for_vehicle(vehicle),load("res://battle/vehicles/battle_arrival_service.gd").door_rect(spec)),"door proxy outside own body")
 for spec in [[5,5,["bayou","bayou"],["interceptor","warden"]],[12,12,["lastlight"],["bulwark","warden"]],[1,1,["yardbird"],["yardbird"]],[12,12,["yardbird","yardbird","yardbird","yardbird","yardbird","yardbird","yardbird","yardbird","yardbird","yardbird","yardbird","yardbird"],["bayou","bayou","bayou"]]]:
  ...
  config.attacker.faction="orlov";config.defender.faction="mercer"
  ...
  if setup.has("battle"):
   ...
   check(not g.get_movement_blocking_obstacle_id_at(Vector2(72,29.5)).is_empty(),"median has physical cover")
   check(g.get_movement_blocking_obstacle_id_at(Vector2(80,29.5)).is_empty(),"median crossing gap open")
   check(Nav.find_path(b,Vector2(72,27),Vector2(72,33)).success,"infantry can route around median to other carriageway")
   check(not g.get_movement_blocking_obstacle_id_at(Vector2(146.5,14)).is_empty(),"defending checkpoint obstructs road")
   check(config.defender.faction=="mercer","non-police defender fixture")
   for v in b.vehicles.values():
    check(v.has_battle_position,"parked "+v.battle_vehicle_id)
    var expected=Vector2(1,1 if v.battle_position.y<29 else -1).normalized() if v.side_id==b.attacker_side_id else Vector2.DOWN
 ...
 FileAccess.open("res://tools/bridge_map/v3_results/validation.json",FileAccess.WRITE).store_string(JSON.stringify({"checks":checks,"errors":errors,"scenarios":scenarios},"  "))
