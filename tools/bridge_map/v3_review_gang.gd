...
var errors=[]
var checks=0
var output="res://tools/bridge_map/v3_results/gang/"
...
func run():
 ...
 scene.custom_loadouts=Cases.make(8,8,["bayou","rancher"])
 scene.custom_loadouts.attacker.faction="orlov";scene.custom_loadouts.defender.faction="mercer"
 scene.open_force_builder();await process_frame
 var panel=scene.surface.get_node("ForceBuilder");panel.choose_map("river_bridge")
 panel.config.defender.vehicles=["bayou","crossway"];panel.changed()
 ...
 for p in b.participants.values():
  if p.side_id==b.attacker_side_id and p.battle_position.distance_to(before[p.participant_id])>1:moved+=1
 view._dusk_zoom=1.55;view._dusk_pan=Vector2(180,-80);view._frame_camera();await capture("bridge_combat")
