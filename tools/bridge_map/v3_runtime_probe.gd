extends RefCounted
static var timings={}
static var timing_counts={}
static func record(name: String,started: int):
	timings[name]=timings.get(name,0.)+(Time.get_ticks_usec()-started)/1000.
...
const BattleCombatBehaviorService := preload("res://tools/bridge_map/v3_combat_probe.gd")
...
static func _advance_validated(battle_state: BattleState, delta_seconds: float) -> BattleRuntimeResult:
	...
	var profile_stamp=Time.get_ticks_usec()
	...
	record("weapon",profile_stamp);profile_stamp=Time.get_ticks_usec()
	...
	record("targets",profile_stamp);profile_stamp=Time.get_ticks_usec()
	...
	record("pressure",profile_stamp);profile_stamp=Time.get_ticks_usec()
	...
	record("tactics",profile_stamp);profile_stamp=Time.get_ticks_usec()
	var combat_result: BattleCombatBehaviorResult = BattleCombatBehaviorService.advance(
		battle_state,
		delta_seconds
	)
	record("combat",profile_stamp);profile_stamp=Time.get_ticks_usec()
	...
	record("path_follow",profile_stamp);profile_stamp=Time.get_ticks_usec()
	...
	record("movement",profile_stamp);profile_stamp=Time.get_ticks_usec()
