...
func build_ui():
	...
	toast = PanelContainer.new(); surface.add_child(toast); toast.custom_minimum_size = Vector2(282,93); toast.hide(); style(toast); toast.mouse_filter = Control.MOUSE_FILTER_IGNORE
	var tm = MarginContainer.new(); tm.mouse_filter = Control.MOUSE_FILTER_IGNORE; toast.add_child(tm)
	for side in ["left","top","right","bottom"]: tm.add_theme_constant_override("margin_"+side,10 if side in ["top","bottom"] else 14)
	toast_label = Label.new(); toast_label.mouse_filter = Control.MOUSE_FILTER_IGNORE; toast_label.custom_minimum_size.x = 254; toast_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; toast_label.add_theme_font_size_override("font_size",16); tm.add_child(toast_label)
...
func show_track_toast():
	...
	toast.scale = Vector2.ONE; toast.modulate.a = 1; toast.show(); dock.hide()
...
func _process(_delta: float):
	...
	toast.position = area-Vector2(toast.size.x+28,toast.size.y+47)
	if panel.visible: toast.position = Vector2(panel.position.x,maxf(12,panel.position.y-toast.size.y-12))
	var elapsed = Time.get_ticks_msec()/1000.0-entry_clock
	if elapsed >= 3.0:
		var q = clampf((elapsed-3.0)/0.75,0.0,1.0)
		if not panel.visible:
			toast.position = toast.position.lerp(dock.position,q); toast.scale = Vector2.ONE.lerp(Vector2(0.436,0.387),q)
...
func advance(preserve_pause: bool) -> bool:
	...
	player.stream_paused = user_paused or (in_sandbox and (not is_instance_valid(sandbox) or not sandbox.ui.visible))
