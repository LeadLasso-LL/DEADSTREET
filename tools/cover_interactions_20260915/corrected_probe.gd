...
func run():
	...
	check(root.get_texture().get_image().save_png(out+"corrected.png")==OK,"native screenshot saved")
	var report={"checks":checks,"errors":errors,"disembark_routes":exits,"model":"veloce","capture":"native paused 12v12 Orlov/Mercer"}
	FileAccess.open(out+"corrected_report.json",FileAccess.WRITE).store_string(JSON.stringify(report,"\t"))
	...
	FileAccess.open(out+"corrected_actors.json",FileAccess.WRITE).store_string(JSON.stringify({"actors":actors,"tight_pairs":tight_pairs},"\t"))
	...
	view._camera.force_update_scroll()
	await settle(.15)
	root.get_texture().get_image().save_png(out+"corrected_stairs_close.png")
	view._camera.position=Vector2(44*8,34*6)
	view._camera.force_update_scroll()
	await settle(.15)
	root.get_texture().get_image().save_png(out+"corrected_vehicles_close.png")
