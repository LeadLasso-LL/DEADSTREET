...
func run():
	out = "res://tools/cover_interactions_20260915/validation/"
	...
	for i in range(all_slots.size()):
		for j in range(i+1,all_slots.size()):
			minimum=minf(minimum,all_slots[i].position.distance_to(all_slots[j].position))
	check(minimum>=2.399,"all Harold cover positions have silhouette clearance")
	for p in b.participants.values():
		check(view.actor_presenter.unit_view_position(p.participant_id).is_equal_approx(p.battle_position*Vector2(8,6)),"drawn foot matches physical origin "+p.participant_id)
	var frame_ms := []
	var advance_ms := []
