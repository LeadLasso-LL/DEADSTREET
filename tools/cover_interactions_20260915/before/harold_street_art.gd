...
func _ready() -> void:
	...
	if not prop.is_empty() and prop[2]=="car" and prop.size()>4:
...
static func cache_bounds(key: String,bounds: Rect2=Rect2()) -> Rect2:
	if key=="ground":return Rect2(-320,-280,1152,280+(H.SIZE.y+2)*6)
...
func ground() -> void:
	...
	box(Rect2(-40,0,144,H.SIZE.y),Color("#30332f"))
	box(Rect2(-40,H.ROAD.position.y,144,H.ROAD.size.y),Color("#252b30"))
	for band in [Rect2(-40,15,144,8),Rect2(-40,H.ROAD.end.y,144,6)]:
	...
	for y in [H.ROAD.position.y,H.ROAD.end.y]:
	...
	for x in range(-320,830,27):
		for y in [H.ROAD.get_center().y*6-1,H.ROAD.get_center().y*6+1]:rect(Rect2(x,y,13,.65),Color("#897044"))
	# Dry asphalt: aggregate and cracks; no wet reflections.
	for i in range(roundi(7500*H.ROAD.size.y/12.)):
		var q=Vector2(rng.randf_range(-70,580),rng.randf_range(H.ROAD.position.y*6+2,H.ROAD.end.y*6-1))
		rect(Rect2(q,Vector2(.45,.35)),Color(.55,.55,.5,rng.randf_range(.035,.13)))
	for i in range(50):
		var q=Vector2(rng.randf_range(0,512),rng.randf_range(H.ROAD.position.y*6+3,H.ROAD.end.y*6-2))
	...
	for q in [Vector2(120,H.ROAD.get_center().y*6+13),Vector2(367,H.ROAD.get_center().y*6-9)]:
	...
	for spot in [Vector2(3.8,16.7),Vector2(16,16.6),Vector2(32.4,16.3),Vector2(39.5,19.2),Vector2(57,16.4),Vector2(4,40.4+H.SOUTH_SHIFT),Vector2(42,40.4+H.SOUTH_SHIFT),Vector2(10.9,37.8+H.SOUTH_SHIFT),Vector2(40.6,37.8+H.SOUTH_SHIFT)]:
		var t=p(spot)
...
func street_car(q: Vector2,sz: Vector2) -> void:
	...
	if arrival and door_open>.02:
		arrival_door(sz.x,height,false,true)
		arrival_door(sz.x,height,false)
	...
	if arrival and door_open>.02:
		...
		arrival_door(sz.x,height,true,true)
...
func arrival_door(width: float,height: float,near_side: bool,rear: bool=false) -> void:
	...
	var hinge=Vector2(width*(-.005 if rear else .205),-height*(.17 if near_side else .715))
	var angle=deg_to_rad(36. if near_side else 48.)*door_open
	var sign_y=1. if near_side else -1.
	var leaf=Vector2(-cos(angle),sign_y*.75*sin(angle))*width*(.16 if rear else .175)
	var tip=hinge+leaf
