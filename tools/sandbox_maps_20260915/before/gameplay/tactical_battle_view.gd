...
func _is_dusk_street() -> bool:
	var g = _geometry()
	return g != null and g.authored_layout_id in ["dead_street_dusk_v1","river_suspension_bridge_v1","whittaker_estate_v1","doble_ocho_yard_v1"]
...
func _sync_dusk_art() -> void:
	...
	for row in preload("res://battle/geometry/dusk_street_catalog.gd").props():
		# Solid stoop walls use one continuous cap and their nearest ground edge.
		var item = art.new()
		item.prop = row
