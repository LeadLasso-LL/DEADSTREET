...
func add_faction_radio(b,side: String,identity: String,candidates: Array,track_id: String):
 ...
 var estate=building and layout in ["whittaker_estate_v1","doble_ocho_yard_v1"]
...
func sync(b,poses: Dictionary,enabled: bool,duck: float,ending: float,battle_mix: float=0.0,victory_mix: float=0.0):
 ...
 for id in sources:
  ...
  var arrival_focus=(1.-clampf(battle_mix,0.,1.)) if b.battlefield_geometry.authored_layout_id=="doble_ocho_yard_v1" and b.battle_phase!="resolved" else 0.
  if is_radio and arrival_focus>0.:
   if v!=null and v.side_id==b.attacker_side_id:
    player.volume_db+=(3.+linear_to_db(1.10))*arrival_focus
    player.max_distance=lerpf(player.max_distance,2600.,arrival_focus)
    player.attenuation=lerpf(player.attenuation,.35,arrival_focus)
