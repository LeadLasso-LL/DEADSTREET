...
func build(b,entrance: Vector2,view=null):
 ...
 var yard=b.battlefield_geometry.authored_layout_id=="doble_ocho_yard_v1"
 var estate=b.battlefield_geometry.authored_layout_id in ["whittaker_estate_v1","doble_ocho_yard_v1"]
 ...
 for i in range(winners.size()):
  ...
  if (bridge or yard) and kind!="check_comrades":
