...
func _ensure_unit_node(battle_state: BattleState, participant: BattleParticipant) -> bool:
	...
	if battle_state.battlefield_geometry.authored_layout_id in ["dead_street_dusk_v1", "river_suspension_bridge_v1", "whittaker_estate_v1", "doble_ocho_yard_v1"]:
...
func _apply_unit_transform(battle_state: BattleState, participant: BattleParticipant) -> void:
	...
	if node == null:
		return
	if battle_state.battle_phase == "resolved" and node.get_meta("outro_owned", false):
		return
	node.position = TacticalParticipantVisual.view_origin(battle_state, participant, pixels_per_unit)
	node.rotation = 0.0
	node.scale = Vector2.ONE * (1.48 if battle_state.battlefield_geometry.authored_layout_id in ["dead_street_dusk_v1", "river_suspension_bridge_v1", "whittaker_estate_v1", "doble_ocho_yard_v1"] else 1.0)
