...
# Poor-neighborhood transport with one conspicuous scarlet Veloce Rosso outside the HQ steps.
const PARKED_CARS = [["north_car_0", "rattleback", 1.0], ["north_car_1", "bayou", 8.8], ["north_car_2", "veloce", 20.3], ["north_car_3", "rancher", 29.6], ["north_car_4", "bayou", 40.5], ["north_car_5", "courier", 54.0], ["south_car_0", "bayou", 1.8], ["south_car_1", "workhorse", 12.1], ["south_car_2", "rattleback", 22.4], ["south_car_3", "bayou", 32.7], ["south_car_4", "civicline", 43.0], ["south_car_5", "rattleback", 53.3]]
...
static func build():
	...
	for row in props():
		...
		var stand=1.35 if kind in ["car","stoop_wall"] else .85
		var points=[[Vector2(center.x,b.position.y-stand),Vector2.DOWN],[Vector2(center.x,b.end.y+stand),Vector2.UP],[Vector2(b.position.x-stand,center.y),Vector2.RIGHT],[Vector2(b.end.x+stand,center.y),Vector2.LEFT]]
		if kind=="stoop_wall":
			...
			points[2][0].y=b.end.y+.35
			points[3][0].y=b.end.y+.35
