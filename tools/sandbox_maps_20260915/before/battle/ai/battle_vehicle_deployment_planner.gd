...
static func _choose_candidate(
	...
	for candidate: Dictionary in candidates:
		...
		if not _candidate_is_free(occupied, position, pose_facing, profile, context.parking_clearance if context != null else 0.6):
			continue
...
static func _candidate_is_free(
	...
	for other: Dictionary in occupied:
		...
		if not Geometry2D.intersect_polygons(BattleVehicleBodyService.cleared_corners(ghost,clearance),BattleVehicleBodyService.cleared_corners(other_ghost,clearance)).is_empty():
			return false
	return true
