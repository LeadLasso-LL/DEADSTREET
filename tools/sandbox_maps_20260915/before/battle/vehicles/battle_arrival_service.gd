...
static func ensure_doors(b,v) -> void:
 ...
 for spec in specs:
  ...
  if g.authored_layout_id in [Catalog.ID,"doble_ocho_yard_v1"] and not _reserve_door_space(b,spec):continue
  g.add_cover_object(CoverObject.new(spec.id,spec.id))
  g.add_cover_slot(Slot.new(spec.id+"_cover",spec.id,spec.slot,spec.facing))
 if g.authored_layout_id in [Catalog.ID,"doble_ocho_yard_v1"]:_space_body_cover(b)
