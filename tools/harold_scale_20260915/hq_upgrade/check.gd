...
func run():
	out = "res://tools/harold_scale_20260915/hq_upgrade/"
	...
	for row in H.props():
		...
		if row[0]=="north_car_2":
			check(row[4]=="volta","HQ uses Volta GT")
			check(row[1].size.is_equal_approx(Vector2(4.65,1.94)*1.6),"Volta actual scaled body")
			check(row[1].position.x<23 and row[1].end.x>27.1,"Volta directly fronts the steps")
	...
	for node in view.dynamic_unit_root.get_children():
		...
		found=true
		check(car.model_id=="volta" and H.VehicleModels.sprite(car.model_id,car.facing,0.)!=null,"Volta canonical sprite renders")
		check(node.position.is_equal_approx(node.prop[1].get_center()*Vector2(8,6)),"Volta artwork/cover ground anchor")
	...
	check(root.get_texture().get_image().save_png(out+"DEAD_STREET_Harold_HQ_Upgrade.png")==OK,"native screenshot saved")
	var report={"checks":checks,"errors":errors,"disembark_routes":exits,"model":"volta","capture":"native paused 12v12 Orlov/Mercer"}
