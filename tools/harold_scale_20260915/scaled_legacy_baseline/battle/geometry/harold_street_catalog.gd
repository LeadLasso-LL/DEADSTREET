...
static func props() -> Array:
	...
	for i in range(6):
		var x: float = [1.,11.6,22.2,32.8,43.4,54.][i]
		rows.append(["north_car_"+str(i),Rect2(x,ROAD.position.y+.7,(5.0+(i%3)*.15)*VehicleModels.TACTICAL_SCALE,PARKED_WIDTH),"car",""])
	for i in range(6):
		var x: float = [1.8,12.1,22.4,32.7,43.,53.3][i]
		rows.append(["south_car_"+str(i),Rect2(x,ROAD.end.y-.6-PARKED_WIDTH,(5.0+(i%3)*.15)*VehicleModels.TACTICAL_SCALE,PARKED_WIDTH),"car",""])
	return rows
static func build():
