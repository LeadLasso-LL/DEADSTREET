...
func run():
 ...
 for i in range(30*240):
  ...
  if i%300==0:
   var sample={"time":b.elapsed_time_seconds,"raiders":d.healthy(b.attacker_side_id).size(),"nbpd":d.healthy(b.defender_side_id).size(),"events":d.seen.size()}
   snapshots.append(sample);print("PACE ",JSON.stringify(sample))
 var report={"seed":seed_value,"phase":b.battle_phase,"winner":b.get_winning_side_id(),"duration":b.elapsed_time_seconds,"commands":d.log,"snapshots":snapshots}
