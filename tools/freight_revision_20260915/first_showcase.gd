...
func run():
 ...
 for row in config.attacker.units:row.tier=3
...
func direct():
 ...
 if t>=.8 and not flags.has("opening"):
  flags.opening=true
  command_group(["sandbox_attacker_unit_01","sandbox_attacker_unit_04","sandbox_attacker_unit_05","sandbox_attacker_unit_07","sandbox_attacker_unit_10"],"push",74.,"Ashford-Crane rifles take the central loading positions")
  command_group(["sandbox_attacker_unit_02","sandbox_attacker_unit_03","sandbox_attacker_unit_06","sandbox_attacker_unit_08","sandbox_attacker_unit_09"],"push",67.,"Assault group advances through the rail crossings")
 if t>=7. and not flags.has("east_flank"):
  flags.east_flank=true
  move("sandbox_attacker_unit_02",Vector2(113.5,56),"SMG flanks along the southern boxcar")
  move("sandbox_attacker_unit_06",Vector2(112,39),"Second SMG moves into the centre lane")
 if t>=12. and not flags.has("counter"):
  flags.counter=true
  var defenders=[]
  for p in b.participants.values():
   if p.side_id==b.defender_side_id and p.weapon_type in ["smg","shotgun"]:defenders.append(p.participant_id)
  command_group(defenders,"push",100.,"McAllister counterattacks to hold the eastern rail crossing")
 if t>=19. and not flags.has("main_push"):
  flags.main_push=true
  command_group(["sandbox_attacker_unit_01","sandbox_attacker_unit_03","sandbox_attacker_unit_04","sandbox_attacker_unit_07","sandbox_attacker_unit_08","sandbox_attacker_unit_09","sandbox_attacker_unit_10"],"push",124.,"Ashford-Crane presses toward dispatch under rifle cover")
 if t>=31. and not flags.has("release"):
  flags.release=true
  for p in b.participants.values():
  ...
  commands.append({"time":t,"action":"Both sides commit their remaining fighters"})
