...
assert report['successful']==17 and not report['failures'], 'Finish source retrieval before catalogue integration'
assert len({t['source_track_id'] for t in report['tracks']})==17,'Duplicate source track requires review'
...
summary={'count':len(results),'expected':17,'failures':failures,'tracks':results}
(OUT/'audio_validation.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
assert len(results)==17 and not failures,failures
...
assert len(catalog['tracks'])==19 and sum('battle_loop' in t for t in catalog['tracks'])==18
...
print('BATCH_ASSETS_AND_CATALOGUE_READY 19 menu /18 battle loops',flush=True)
