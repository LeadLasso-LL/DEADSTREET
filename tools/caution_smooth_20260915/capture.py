...
config=re.sub(r'^config/name=.*$', 'config/name="DeadStreetIntroSmooth915"',config,flags=re.M)
config+='\n[editor]\nmovie_writer/mjpeg_quality=0.95\n'
(project/'project.godot').write_text(config,encoding='utf-8')
script=project/'tools/caution_smooth_20260915/review.gd';script.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(o/'review.gd',script)
...
args=[godot,'--path',str(project),'--resolution','1280x720','--position','0,0','--fixed-fps','60','--disable-vsync','--write-movie',str(raw),'--log-file',str(o/'native_engine.log'),'--script','res://tools/caution_smooth_20260915/review.gd']
...
final=o/'DEAD_STREET_Intro_Smooth_Zoom.mp4'
