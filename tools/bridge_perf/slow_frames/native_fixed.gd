...
func run():
 ...
 scene.ready_started=true;scene.set_process(false)
 ...
 for fixed_step in range(1800):
  await process_frame
  var sim_start=Time.get_ticks_usec()
  scene._process(1./60.)
  simulation_us+=Time.get_ticks_usec()-sim_start
  var alive=0
  for p in b.participants.values():
   if p.is_alive:alive+=1
  roster_frames.append(alive)
 ...
 var state_rows=[b.combat_random.snapshot_state(),b.battle_phase]
 for p in b.participants.values():state_rows.append([p.participant_id,p.is_alive,p.vitality,p.battle_position,p.target_participant_id,p.weapon_state.ammo_in_magazine])
 report["final_state"]=var_to_str(state_rows)
 report["fixed_steps"]=1800;report["simulation_ms_per_step"]=simulation_us/1800000.
 report["full_roster_frames"]=roster_frames.count(n*2)
 ...
 FileAccess.open("C:/Users/brand/OneDrive/Documents/dead-street/tools/bridge_perf/slow_frames/"+label+".json",FileAccess.WRITE).store_string(JSON.stringify(report,"  "))
