extends SceneTree
const Meter=preload("res://tools/bridge_perf/slow_frames/meter.gd")
...
func run():
 ...
 var traces=[]
 Meter.totals.clear();Meter.frame.clear();Meter.enabled=true
 ...
 while Time.get_ticks_usec()-started<int(duration*1000000) and b.battle_phase=="active":
  ...
  traces.append({"frame_ms":(now-previous)/1000.,"sim":b.elapsed_time_seconds,"alive":alive,"stages_us":Meter.frame.duplicate()})
  Meter.frame.clear()
  frames.append((now-previous)/1000.);previous=now;frame_count+=1
 ...
 report["trace"]=traces;report["profiles"]=Meter.totals
 ...
 print("TRACE_DONE ",label," fps=",report.fps," p95=",report.p95_frame_ms);scene.queue_free();await process_frame;quit()
