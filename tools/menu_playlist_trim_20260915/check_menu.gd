...
const OUT="res://tools/menu_playlist_trim_20260915/"
func _initialize():call_deferred("run")
func run():
	var removed=["switch_b22","ripper_b22","dead_or_alive_b22","lurk_b22"]
	var settings="user://menu_trim_validation.cfg"
	...
	await process_frame
	var ok=menu.tracks.size()==18 and menu.track_checks.size()==18 and Catalog.tracks().size()==22
	for id in removed:
	...
	print("MENU_PLAYLIST_TRIM ","PASS" if ok else "FAIL")
	quit(0 if ok else 1)
