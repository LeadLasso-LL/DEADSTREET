...
plan=json.loads((O/'publication_plan.json').read_text());docs=json.loads((O/'owned_docs.json').read_text())
assert not t('diff','--cached','--name-only'),'Concurrent index owner'
...
assert sha(R/'gameplay/sandbox_menu_music.gd')==plan['live_menu_hash']
head=t('rev-parse','HEAD');scope=plan['scope']+['tools/menu_playlist_trim_20260915/publication_plan.json']
...
assert set(t('diff','--cached','--name-only').splitlines())<=set(scope)|set(docs)
assert head==t('rev-parse','HEAD')
# Unified-diff context whitespace is required patch syntax.
g('diff','--cached','--check','--','.',':(exclude)tools/menu_playlist_trim_20260915/applied_menu_delta.patch')
g('-c','gc.auto=0','commit','-m','Exclude four tracks from sandbox menu without changing faction audio')
commit=t('rev-parse','HEAD');receipt={'state':'committed locally; push pending','commit':commit,'parent':head,'native_validation':'PASS','menu_tracks':18,'unchanged_audio_files':43,'destination':t('remote','get-url','origin')};(O/'publication_receipt.json').write_text(json.dumps(receipt,indent=2))
