...
var commands=[]
var errors=[]
var output="res://tools/fourth_map_20260915/"
var advance_times=[]
var stage_frames={}
var seat_manifest=[]
var capture=false
...
func shot(name_value: String):
 ...
 if name_value=="combat_14":
  for item in view._dusk_nodes:
   if item.get("prop")!=null and not item.prop.is_empty() and item.prop[2]=="container":
    print("CONTAINER_DEBUG ",item.position," ",item.self_modulate," process ",item.is_processing())
  for unit in b.participants.values():
   if unit.battle_position.x>45 and unit.battle_position.x<60:
    var node=view.actor_presenter._unit_nodes.get(unit.participant_id)
    print("ACTOR_DEBUG ",unit.participant_id," ",unit.battle_position," ",node.position," alive ",unit.is_alive)
  quit()
...
func direct():
 ...
 if t>=1.2 and not flags.has("flank_move"):
  ...
  move("sandbox_attacker_unit_05",Vector2(73.5,48),"Service team moves outside the fence")
  move("sandbox_attacker_unit_06",Vector2(77,49.5),"Service team takes the second gate")
  Commands.issue(b,["sandbox_attacker_unit_07"],"hold",69.)
 if t>=5.2 and not flags.has("gate_entry"):
  flags.gate_entry=true
  move("sandbox_attacker_unit_05",Vector2(73.5,37),"Flanker enters through the open service gate")
  move("sandbox_attacker_unit_06",Vector2(77.5,35.5),"SMG takes the service corner")
 if t>=7.5 and not flags.has("main_push"):
  flags.main_push=true
  var result=Commands.issue(b,["sandbox_attacker_unit_01","sandbox_attacker_unit_02","sandbox_attacker_unit_03","sandbox_attacker_unit_04"],"push",59.)
 ...
 if t>=16. and not flags.has("counter"):
  ...
  var result=Commands.issue(b,defenders,"push",58.)
