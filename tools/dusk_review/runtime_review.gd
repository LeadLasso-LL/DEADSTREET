...
func start() -> void:
	...
	for p in battle.participants.values():
		...
		var placed := false
		var preferred := Vector2(32 + occupied.size()*3,42)
	...
	for slot_id in g.get_sorted_cover_slot_ids():
		...
		if not nav.is_reachable(battle,Vector2(35,42),slot.position):
			problems.append("unreachable "+slot_id)
	var threshold = g.get_cover_slot("cover_threshold_sedan_1").position
	var defensive = g.get_cover_slot("cover_club_wall_w_0").position
