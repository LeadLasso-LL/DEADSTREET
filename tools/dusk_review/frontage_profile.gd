...
var active := false
var comparing := false
var out_dir := "res://tools/dusk_review/frontage_profile_results"
...
func start() -> void:
	...
	if not problems.is_empty():
		fail("layout audit")
		return
	if "--old-wall-los" in OS.get_cmdline_user_args():
...
func _process(delta: float) -> bool:
	...
	if battle.battle_phase == "active":
		...
		if elapsed>3:cpu_samples.append((Time.get_ticks_usec()-before)/1000.)
