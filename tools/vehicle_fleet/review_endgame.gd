...
var errors=[]
var review
var output="res://tools/vehicle_fleet/endgame_review/"
...
func run():
	...
	for scenario in [{"name": "flagship_bikes", "faction": "stateline", "vehicles": ["wraith_zero", "crownfire", "crownfire"]}, {"name": "flagship_cars", "faction": "ravicci", "vehicles": ["asterion", "eidolon"]}, {"name": "flagship_utility", "faction": "wm_corp", "vehicles": ["nomad", "archangel"]}, {"name": "flagship_heavy", "faction": "trc", "vehicles": ["leviathan", "palisade"]}, {"name": "bank_services", "faction": "nbpd", "vehicles": ["sterling_cit", "sterling_reserve"]}, {"name": "prison_service", "faction": "nbpd", "vehicles": ["custodian", "custodian"]}]:
	...
	review.open_vehicle_fleet()
	var final_panel=review.surface.get_node("VehicleFleet");final_panel.open_encounter_lab()
	var lab=final_panel.get_node("EncounterLab")
	await process_frame;lab.picker.select(7);lab.execute(false);lab.execute(false);await capture("encounter_lab")
