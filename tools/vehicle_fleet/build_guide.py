...
'sand_raiders':'Scavenged bicycles, battered compacts, pickups, worn motorhomes, and retired buses.',
...
for key,info in DATA['classes'].items():
 ...
 for offset in range(0,len(group),4):
  ...
  for j,m in enumerate(group[offset:offset+4]):
   ...
   x=363;text(x,top+12,m['name'],25,FG,'Bold')
   ...
   para(x,top+163,m['description'],470,17,MUTED,23)
  if count<4:
   yy=160+count*249
