...
def start(title,subtitle):
 ...
 rect(34,1190,832,1,BORDER);text(35,1210,'75 models / 4 classes / Fictional game specifications',13,MUTED)
 text(819,1208,f'{page:02d}',16,GOLD,'Bold')
def end():c.showPage()
start('The 2034 fleet','75 vehicles / Campaign transport and tactical arrivals.')
...
for i,(key,title,n,cap) in enumerate([('two_wheelers','Two-Wheelers',15,'1-2'),('passenger_cars','Passenger Cars',22,'1-4'),('utility_vehicles','Utility Vehicles',16,'1-7'),('heavy_transports','Heavy Transports',22,'1-12')]):
...
lines=['# Dead Street - Vehicle Fleet','', '75 fictional vehicle models across four classes. Initial balance values; all models unlocked in the battle sandbox.','', 'Seats include the driver. One unit is reserved to drive each vehicle. Only Heavy Transports carry campaign resources. Convoy road speed is its slowest vehicle.','']
for key,info in DATA['classes'].items():
...
lines+=['## Endgame abilities','', 'Twelve premium vehicles have encounter rules. The Encounter Lab is available from the fleet selector. Roadwarden and Bloodhound can launch controlled tactical battles from the lab. Automatic campaign event generation remains future integration work.', '', '| Vehicle | Ability | Effect | Limits |', '|---|---|---|---|']
for m in MODELS.values():
...
draw.text((47,88),'75 models - consistent illustration scale - 4 transport classes',fill=MUTED,font=ImageFont.truetype(str(regular),22))
