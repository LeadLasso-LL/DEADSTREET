...
assert report['checks']>=3000 and not report['errors']
ffmpeg=imageio_ffmpeg.get_ffmpeg_exe()
target=out/'Dead_Street_Sandbox_Panels_Preview.mp4'
args=[ffmpeg,'-hide_banner','-y','-i',str(out/'glossary_raw.avi'),'-t',str(report['preview_end_seconds']),'-an','-c:v','libx264','-preset','medium','-tune','animation','-crf','19','-pix_fmt','yuv420p','-r','30','-movflags','+faststart',str(target)]
