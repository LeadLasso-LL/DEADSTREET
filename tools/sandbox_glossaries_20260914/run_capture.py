...
stage = Path(r'C:\Users\brand\AppData\Local\DeadStreetTools\sandbox_glossaries_20260914')
...
snapshot = stage / 'source_snapshot_v1'
...
for folder in ('battle','campaign','core','gameplay'):
    for path in (repo/folder).rglob('*'):
        ...
        if not dest.exists() or relative in ('gameplay/arsenal_review.gd','gameplay/sandbox_force_builder.gd','gameplay/sandbox_menu_panels.gd','gameplay/sandbox_glossary_panel.gd'):
...
for path in (repo/'assets/art').rglob('*.png'):
    if 'portraits' not in path.parts and not path.name.endswith('_portraits.png') and 'icons' not in path.parts: continue
    files[path.relative_to(repo).as_posix()] = str(path)
...
for path in (repo/'assets/data').glob('*.json'): files[path.relative_to(repo).as_posix()]=str(path)
files['tools/sandbox_glossaries_20260914/'+script_name] = str(out/script_name)
boot = '''extends Node
func _ready():
 if not ProjectSettings.load_resource_pack(DATA_PACK,false):get_tree().quit(5);return
 var tree=get_tree()
 tree.set_script(load("res://tools/sandbox_glossaries_20260914/SCRIPT_NAME"))
...
print('GLOSSARY_RUN_COMPLETE',mode,flush=True)
