"""Publish only owned tutorial content while retaining concurrent doc edits."""
...
subprocess.run(['git','push','origin','HEAD:build/arsenal-checkpoint-20260911'],cwd=repo,check=True)
remote=git('ls-remote','origin','refs/heads/build/arsenal-checkpoint-20260911').split()[0]
assert remote==head,(head,remote)
receipt=dict(status='PUSHED / VERIFIED',commit=head,remote=remote,branch='build/arsenal-checkpoint-20260911',files=staged,checks=73,errors=[],shared_release_runtime_modified=False,preview_library_id='libfile_d56b156bea7c81919ef35265ee17c7a9')
