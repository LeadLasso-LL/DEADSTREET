...
report={}
for kind,decay,low,high,body_gain in [('smg',.014,220,13500,.32),('rifle',.023,130,16500,.50),('pistol',.016,200,14500,.30),('shotgun',.039,80,12500,.65)]:
 for i in range(3):
  t=np.arange(int(SR*.85))/SR
  # Short broadband pressure impulse, irregular low body, with a dry mechanical tail.
  ...
  dry/=max(abs(dry))
  out=dry.copy()
  # Dry report: remove the diffuse snare-like wash and delayed reverberant repeats.
  out+=band(rng.normal(size=len(t)),900,6000)*.004*np.exp(-t/.045)
...
(R/'WEAPON_REVISION.json').write_text(json.dumps({'revision':3,'source':'Original procedural waveforms; no external samples','seed':20260910,'assets':report},indent=2))
