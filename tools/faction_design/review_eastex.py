...
def generate(extended=False):
    ...
    def frame(role,direction,clip,i,model=None):
        ...
        costume=style.outfits.SPECS[(0,model['art_base'])]
        style.outfits.SPECS[(0,model['art_base'])]=style.BASE
        reference=style.directions.make(0,st['q'],direction,model['art_base'],**{k:v for k,v in st.items() if k!='q'})
        assert style.rig.body_signature(root)==style.rig.body_signature(reference),(key,'body geometry')
        style.outfits.SPECS[(0,model['art_base'])]=costume
        geometry+=1
    ...
    (OUT/'geometry_validation.json').write_text(json.dumps({'frames':geometry,'body_matches_accepted_geometry':True,'extended':extended},indent=2)+'\n')
    print('EASTEX_SVG',len(jobs),flush=True)
...
def board():
    ...
    im=Image.new('RGB',(1440,824),'#151c20');header(im,"EASTEX 44'S",'Regular roster / standing and aiming / same rig and fixed canvas scale')
    d=ImageDraw.Draw(im)
    ...
    im.save(OUT/'eastex_lineup_review.png')
    im=Image.new('RGB',(800,2030),'#151c20')
    header(im,"EASTEX 44'S",'Five regular outfits / standing and aiming')
    d=ImageDraw.Draw(im)
    ...
    im.save(OUT/'eastex_outfit_review.png')
    for clip in ['idle','aim']:
        im=Image.new('RGB',(1664,1232),'#151c20')
        header(im,"EASTEX 44'S / "+('STANDING' if clip=='idle' else 'AIMING'),'All eight directions / fixed 128px source, enlarged equally')
        d=ImageDraw.Draw(im)
        for row,role in enumerate(ROLES):
            y=104+row*224
        ...
        im.save(OUT/f'eastex_{clip}_directions.png')
    print('EASTEX_BOARDS',len(jobs),flush=True)
    ...
    for clip,count in [('walk',24),('fire',3),('reload',40),('cover_popout',8),('cover_tuck',9),('wounded_walk',24)]:
        for i in range(count):
            ...
            header(im,"EASTEX 44'S / "+clip.replace('_',' ').upper(),'Existing animation clips / five regular outfits')
            d=ImageDraw.Draw(im)
            for col,role in enumerate(ROLES):
        ...
        durations[-1]+=250
    frames[0].save(OUT/'eastex_motion_review.gif',save_all=True,append_images=frames[1:],duration=durations,loop=0,disposal=2)
    print('EASTEX_MOTION',len(frames),flush=True)
...
def audit():
    ...
    header(im,"EASTEX 44'S / MOTION SAMPLES",'Fixed-scale SE stance samples from the standard clips')
    d=ImageDraw.Draw(im)
    for row,role in enumerate(ROLES):
        y=104+row*224
        d.text((18,y),role.upper(),font=font(16,True),fill='#efeadc')
    ...
    im.save(OUT/'eastex_motion_samples.png')
    models=list(art.MODELS)
    im=Image.new('RGB',(1536,1416),'#151c20')
    header(im,"EASTEX 44'S / ARMORY FIT",'All 30 existing models / aiming pose / fixed canvas scale')
    d=ImageDraw.Draw(im)
    for row,role in enumerate(ROLES):
    ...
    im.save(OUT/'eastex_armory_fit.png')
    print('EASTEX_AUDIT_BOARDS',flush=True)
