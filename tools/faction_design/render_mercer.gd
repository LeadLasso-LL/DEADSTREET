...
func _initialize():
    var base = "res://tools/faction_design/mercer/"
    var jobs: Array = JSON.parse_string(FileAccess.get_file_as_string(base + "jobs.json"))
    var failures: Array = []
    var bounds: Dictionary = {}
    for key: String in jobs:
        var im = Image.new()
        var err = im.load_svg_from_string(FileAccess.get_file_as_string(base + key + ".svg"), 2.0)
        if err != OK:
        ...
        if im.save_png(base + key + ".png") != OK:
            failures.append(key + ": PNG save failed")
    var report = {"frames": jobs.size(), "failures": failures, "bounds": bounds}
    FileAccess.open(base + "render_validation.json", FileAccess.WRITE).store_string(JSON.stringify(report, "  "))
    print("MERCER_PREVIEW_RENDER ", JSON.stringify(report))
