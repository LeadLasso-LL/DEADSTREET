...
func run():
 ...
 root.size=Vector2i(1920,1080);DisplayServer.window_set_size(root.size);await settle(4)
 var b=scene.battle;var view=scene.runtime.get_node("TacticalBattleView");var p=view.battle_presentation
 p.reset(b);p.begin_arrival();p._process(0.);p.set_process(false);p.visible=true;p.stage="arrival";p.clock=8.;p.apply_poses();p.update_markers()
 ...
 print("VIEWPORT_DIAG ",root.size," visible=",root.get_visible_rect()," scale_size=",root.content_scale_size," factor=",root.content_scale_factor," mode=",root.content_scale_mode," window=",DisplayServer.window_get_size()," screen=",DisplayServer.screen_get_size()," view=",view.get_viewport_rect()," path=",view.get_viewport().get_path())
 var marker_rows=[]
 for badge in p.markers.values():
  var box=view.get_viewport().get_stretch_transform()*badge.get_global_rect();var native_size=badge.emblem.texture.get_size()
  check(box.size.distance_to(Vector2(36,36))<.01,"36px badge at1080p")
  check(box.position.distance_to(box.position.round())<.01,"badge lands on output pixels")
  check(native_size.distance_to(box.size)<.01,"one texture pixel per screen pixel")
 ...
 for badge in p.markers.values():check((view.get_viewport().get_stretch_transform()*badge.get_global_rect()).size.is_equal_approx(Vector2(24,24)),"24px badge at720p")
