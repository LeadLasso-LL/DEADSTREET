...
var commands=[]
var errors=[]
var output="res://tools/freight_stability_20260915/"
var advance_times=[]
var stage_frames={}
var seat_manifest=[]
var capture=false
func _initialize():
 var monitor=MotionMonitor.new();monitor.owner_script=self;root.add_child.call_deferred(monitor)
 call_deferred("run")
func run():
 ...
 root.size=Vector2i(1920,1080);DisplayServer.window_set_size(root.size);await process_frame
...
class MotionMonitor extends Node:
 var owner_script
 var rows=[]
 var last_bounds=Rect2()
 var last_safe=Rect2()
 var last_camera=Vector3()
 var stable_samples=0
 var unstable_samples=0
 var body_offset_errors=0
 func _ready():process_priority=2000
 func _process(_delta):
  var s=owner_script
  if not is_instance_valid(s.view) or not is_instance_valid(s.presentation):return
  var camera=s.view._camera;var safety=s.view.get_node("CameraSafety")
  var now=Vector3(camera.position.x,camera.position.y,camera.zoom.x)
  if s.presentation.stage=="active":
   if not rows.is_empty() and safety.last_bounds.is_equal_approx(last_bounds) and safety.last_safe.is_equal_approx(last_safe):
    stable_samples+=1
    if now.distance_to(last_camera)>.0001:unstable_samples+=1
   for node in s.view.actor_presenter._unit_nodes.values():
    if node.get_node("body").position.length()>.0001:body_offset_errors+=1
   rows.append([s.b.elapsed_time_seconds,now.x,now.y,now.z])
  last_bounds=safety.last_bounds;last_safe=safety.last_safe;last_camera=now
 func _exit_tree():
  var report={"stable_bounds_frames":stable_samples,"unexpected_camera_changes":unstable_samples,"body_offset_errors":body_offset_errors,"camera_samples":rows}
  FileAccess.open("res://tools/freight_stability_20260915/full_motion.json",FileAccess.WRITE).store_string(JSON.stringify(report))
