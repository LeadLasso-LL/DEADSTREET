...
def render(s):
    ...
    base=f'{s.number:02d}_{s.id}_r2'
    stage=Path(tempfile.mkdtemp(prefix='dead_street_r2_'))
    ...
    subprocess.run(['ffmpeg','-v','error','-y','-i',str(master),'-c:a','libmp3lame','-b:a','192k','-metadata','title='+s.name+' — dark rebuild 02','-metadata','artist=DEAD STREET','-metadata','album=DEAD STREET — dark rebuild 02 — awaiting review','-metadata','track='+str(s.number),str(mp3)],check=True)
    raw.unlink()
