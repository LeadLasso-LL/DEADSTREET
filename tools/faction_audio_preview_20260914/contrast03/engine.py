...
def render(s):
    ...
    base=f'{s.number:02d}_{s.id}_c3'
    stage=Path(tempfile.mkdtemp(prefix='dead_street_c3_'))
    ...
    subprocess.run(['ffmpeg','-v','error','-y','-i',str(master),'-c:a','libmp3lame','-b:a','192k','-metadata','title='+s.name+' — contrast 03','-metadata','artist=DEAD STREET','-metadata','album=DEAD STREET — contrast 03 — awaiting review','-metadata','track='+str(s.number),str(mp3)],check=True)
    raw.unlink()
