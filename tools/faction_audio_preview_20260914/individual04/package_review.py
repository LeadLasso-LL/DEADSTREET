...
assert len(rows)==18 and {x['number'] for x in rows}==set(range(1,19))
...
def card(r):
    ...
    d.text((65,94),'FACTION AUDIO  /  INDIVIDUAL SCORES 04',font=f(17),fill=(143,146,140))
    d.text((1070,47),f'{num:02d} / 18',font=f(27,True),fill=accent)
    ...
    vibe_lines=[]; current=''
    for word in r['vibe'].split():
        trial=(current+' '+word).strip()
        if d.textlength(trial,font=f(24))>1100:
            vibe_lines.append(current); current=word
        else: current=trial
    vibe_lines.append(current)
    for i,line in enumerate(vibe_lines): d.text((68,yy+24+i*34),line,font=f(24),fill=accent)
    assert yy+24+len(vibe_lines)*34<465, r['name']
    ...
    d.text((65,657),('PRESERVED RECORDING — DIRECTION LIKED' if r.get('origin','').startswith('contrast03') else 'NEW INSTRUMENTAL SKETCH — AWAITING YOUR REVIEW'),font=f(16),fill=(122,128,123))
...
print('18 labeled video clips rendered',flush=True)
concat=OUT/'concat.txt'; concat.write_text(''.join("file '"+str(c)+"'\n" for c in clips))
meta=[';FFMETADATA1','title=DEAD STREET — Eighteen Individual Faction Scores','artist=DEAD STREET','comment=New dark compositions after rejected audition01. Tempos 20 percent lower. Preview only; pending owner approval.']
t=0
for r in rows:
...
video=OUT/'Dead_Street_All_Factions_Individual04.mp4'
...
(OUT/'Dead_Street_All_Factions_Individual04_Player.html').write_text(page)

def verify(r):
    p=PRE/r['mp3']
    assert hashlib.sha256(p.read_bytes()).hexdigest()==r['sha256'], r['id']
    sr,pcm=wavfile.read(PRE/r['wav'])
    assert sr==44100 and pcm.ndim==2 and pcm.shape[1]==2
    assert abs(len(pcm)/sr-r['duration'])<.0001
    assert pcm.dtype==np.int16 and abs(pcm.astype(np.int32)).max()<32760
...
lines=['# DEAD STREET — eighteen individual faction scores','', 'Instrumental sketches for listening review. No runtime integration; no approved soundtrack assets.','', '| # | Faction | Reel start | Vibe |','|---|---|---|---|']
for r in rows:
