extends SceneTree
const OUT="C:/Users/brand/OneDrive/Documents/dead-street/tools/sandbox_finish_20260915/"
...
func move(point: Vector2):
	var e=InputEventMouseMotion.new();e.position=point;e.global_position=point;root.push_input(e,true);await frames(3)
...
func run():
	...
	await item(arsenal,"glock_17");await shot("arsenal_glock");await class_tab(arsenal,"smg");await item(arsenal,"uzi");await shot("arsenal_uzi");await class_tab(arsenal,"pistol")
	...
	for id in Factions.all_ids():
		...
		for role in factions.CLASSES:
			...
			check(picture.texture.get_size()==Vector2(90,80),"uniform portrait canvas "+id+" "+role)
	...
	f.store_string(JSON.stringify({"checks":checks,"errors":errors,"preview_end_seconds":preview_end_seconds,"factions":23,"paired_unit_images":115,"all_portrait_loadouts":691,"leader_photos":20,"weapons":guns,"vehicles":cars,"battle_launched_after_preview_only":true},"\t"));f.close()
