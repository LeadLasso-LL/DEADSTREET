...
## 20260915-cover-slot-picker-proposal-01 - Hold to choose an exact cover position

Status: PROPOSED / discussion only. User asked whether this interaction works; no gameplay implementation in this block.
User intent: preserve autonomous cover management and quick-click automatic cover orders. With a selected unit, holding on cover reveals small silver/white ground circles for usable slots; mouse movement highlights a candidate; release orders that exact position. Temporarily fade the object so hidden/far-side choices are visible. Large freight sections motivated the request.
Code assessment: existing legal-slot ranking plus navigation reachability checks can supply candidates. Existing reserve/path/occupy flow can be reused with an explicit slot choice. Freight boxcars/timber cars currently expose corner/end positions, not continuous full-length side positions; additional slots would be a separate geometry decision.
Suggested interaction details (not locked): short hold threshold around 0.25 seconds; distinguish cover-picking from selection-box dragging; highlight nearest candidate and preview the route immediately even while tactically paused; reserve only on release after revalidation; Escape/right-click cancels without issuing an order; restore opacity when closing. Preserve existing emblem input priority and coordinate cover fading with current hover/occlusion effects.
Validation: source inspection only; no new gameplay edits or runtime tests. Next step: implement and validate the gesture after user direction, including pause, invalid/occupied slots, cancellation, and long freight cover.

## 20260915-cover-slot-picker-01 - Exact cover picker and all-command audio authorized
Status IN PROGRESS. Owner approved implementing the prior cover picker proposal, clarified all slots use one silverish-white tone, and requested that only the confirmed slot linger about half a second then fade. Selected-unit yellow paths persist independently. Every registered command must sound, including repeated replacements while paused; single-unit cue should resemble the accepted group cue with a subtle variation.
Scope: tactical_orders_controller.gd, tactical_command_feedback.gd, tactical_order_audio.gd and a small picker presentation helper if needed. Preserve release-owner arrival/audio sources, current AI cover behavior, existing geometry slots and packaged release snapshot. Inspection found direct move/cover/target orders currently bypass group acknowledge/audio. Next: explicit slot validation, hold gesture with cancellation, faded-object slot presentation, shared acknowledgment, focused paused/live tests. Baseline tools/cover_slot_picker_20260915/.

## 20260915-cover-slot-picker-02 - Implemented and five-map interaction checks passed
IMPLEMENTED / VALIDATION IN PROGRESS. Exact single-unit cover picker opens after 0.25s, shows reachable legal existing slots in silverish white, previews yellow route, fades actual cover art, and commits exact slot on release. Other slots disappear immediately; confirmed marker holds 0.5s then fades over 0.25s on presentation time, including pause. No geometry/AI changes. Escape/right-click/HUD release/focus loss cancels; early drag retains box selection; badges retain picking priority.
Move/cover/target now use the same acknowledgment path as group commands. Distinct queued events avoid losing rapid paused inputs; accepted single-unit cues use the existing mechanical/two-tone receipt with subtle 1.08 pitch, groups 1.0. Existing audio toggle and failure cue preserved.
145 focused native checks PASS across all five maps; 58 actual OpenGL renderer checks PASS on Harold/Freight. Evidence in tools/cover_slot_picker_20260915/. Readability screenshots reviewed. Final railcar-specific check and constant marker color under night lighting underway. An editor scan initially indexed historical before-copies as live global classes; evidence folders now have .gdignore, and the two erroneous local cache entries were restored to canonical paths. First failing log preserved. No fresh performance benchmark or packaged-release update implied.

## 20260915-cover-slot-picker-03 - Exact cover picking and command sounds validated
IMPLEMENTED / NATIVE AND RENDERER VALIDATED; owner playtest acceptance pending. All246 focused checks PASS:145 across all five maps,58 actual-renderer Harold/Freight checks,43 railcar-specific renderer/audio/edge-case checks. Railcar case confirmed four corner slots, exact nonautomatic choice, stale reservation failure retaining the old order, already-occupied exact-slot confirmation, emblem priority, focus cleanup, wounded exclusion, muted-event draining, and nonzero unclipped generated cues. Reviewed actual1280x800 screenshots; silverish-white markers remain clear in freight rain and use unshaded ink to avoid night-color shifts. Circle persists0.5s then fades0.25s; selected-unit path remains.
Canonical interaction rules: docs/TACTICAL_COVER_PICKER.md. Production changes are limited to tactical_orders_controller.gd, tactical_command_feedback.gd, tactical_order_audio.gd and new tactical_cover_picker.gd. Before-copy evidence folders excluded from editor indexing to prevent duplicate global classes. Fresh source hashes/results in tools/cover_slot_picker_20260915/validation.json.
No AI/cover geometry, faction music, convoy or intro/montage changes. Other release-owner source edits preserved. No new16v16 FPS claim. Live-source sandbox includes this on relaunch; existing standalone package is deliberately not rebuilt while owner continues playtest fixes. Next: scoped commit/push to established origin/build branch, then owner playtest.

## 20260915-cover-slot-picker-04 - Local completion; publication blocked by automatic approval review
Implementation and246 focused native/renderer checks are complete. Automatic approval review REJECTED the scoped combined commit/push before execution. Exact stated reason: commits and exports source code plus large generated evidence artifacts to an externally hosted Git remote whose ownership/trust and destination were not explicitly established by the user. Existing standing authorization is recorded, but this tool decision remains binding; no bypass or network retry.
Complete unaffected LOCAL COMMIT ONLY. Intended publication: this exact cover-picker/audio fix, documentation and34-file validation/evidence scope, to https://github.com/LeadLasso-LL/DEADSTREET.git branch build/arsenal-checkpoint-20260911. Source is live locally; standalone package is not updated. Ask owner for explicit approval for that concrete commit/destination and disclose auto-review's stated reason. Local receipt: tools/repository_checkpoint_20260915/local/cover_picker_local_commit.json. Other chats' unstaged source/records preserved.
